...
void HandleSetAsrAecModeFuzzTest()
{
    GetPermission();
    std::shared_ptr<AudioServer> audioServer = std::make_shared<AudioServer>(SYSTEM_ABILITY_ID, RUN_ON_CREATE);
    MessageParcel data;
    MessageParcel reply;
    audioServer->HandleSetAsrAecMode(data, reply);
}

void HandleGetAsrAecModeFuzzTest()
{
    GetPermission();
    std::shared_ptr<AudioServer> audioServer = std::make_shared<AudioServer>(SYSTEM_ABILITY_ID, RUN_ON_CREATE);
    MessageParcel data;
    MessageParcel reply;
    audioServer->HandleGetAsrAecMode(data, reply);
}

void HandleFourthPartCodeFuzzTest()
{
    GetPermission();
    uint32_t audioServerInterfaceCodeCount = GetData<uint32_t>() % AudioServerInterfaceCodeVec.size();
    AudioServerInterfaceCode audioServerInterfaceCode = AudioServerInterfaceCodeVec[audioServerInterfaceCodeCount];
    ...
    audioServerN->HandleFourthPartCode(format, data, reply, option);
}

void HandleFifthPartCodeFuzzTest()
{
    GetPermission();
    std::shared_ptr<AudioServer> audioServer = std::make_shared<AudioServer>(SYSTEM_ABILITY_ID, RUN_ON_CREATE);
    uint32_t audioServerInterfaceCodeCount = GetData<uint32_t>() % AudioServerInterfaceCodeVec.size();
    ...
    MessageParcel reply;
    MessageOption option;
    audioServer->HandleFifthPartCode(format, data, reply, option);
}

void HandleSixthPartCodeFuzzTest()
{
    GetPermission();
    std::shared_ptr<AudioServer> audioServer = std::make_shared<AudioServer>(SYSTEM_ABILITY_ID, RUN_ON_CREATE);
    uint32_t audioServerInterfaceCodeCount = GetData<uint32_t>() % AudioServerInterfaceCodeVec.size();
    ...
    MessageParcel reply;
    MessageOption option;
    audioServer->HandleSixthPartCode(format, data, reply, option);
}

void HandleThirdPartCodeFuzzTest()
{
    GetPermission();
    std::shared_ptr<AudioServer> audioServer = std::make_shared<AudioServer>(SYSTEM_ABILITY_ID, RUN_ON_CREATE);
    uint32_t audioServerInterfaceCodeCount = GetData<uint32_t>() % AudioServerInterfaceCodeVec.size();
    ...
    MessageParcel reply;
    MessageOption option;
    audioServer->HandleThirdPartCode(format, data, reply, option);
}

void HandleSecondPartCodeFuzzTest()
{
    GetPermission();
    std::shared_ptr<AudioServer> audioServer = std::make_shared<AudioServer>(SYSTEM_ABILITY_ID, RUN_ON_CREATE);
    uint32_t audioServerInterfaceCodeCount = GetData<uint32_t>() % AudioServerInterfaceCodeVec.size();
    ...
    MessageParcel reply;
    MessageOption option;
    audioServer->HandleSecondPartCode(format, data, reply, option);
}

TestFuncs g_testFuncs[TESTSIZE] = {
    OnRemoteRequestFuzzTest,
    HandleSetAsrAecModeFuzzTest,
    HandleGetAsrAecModeFuzzTest,
    HandleFourthPartCodeFuzzTest,
    HandleFifthPartCodeFuzzTest,
    HandleSixthPartCodeFuzzTest,
    HandleThirdPartCodeFuzzTest,
    HandleSecondPartCodeFuzzTest,
};
