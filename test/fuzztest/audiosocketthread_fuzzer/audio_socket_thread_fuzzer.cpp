...
void AudioSocketThreadAudioAnahsDetectDeviceFuzzTest()
{
    ...
    if (isNull) {
        ...
        bool isSetAnahsName = GetData<bool>();
        if (isSetAnahsName) {
...
void AudioSocketThreadAudioHDMIDetectDeviceFuzzTest()
{
    ...
    AudioPnpUevent uevent = {
        .subSystem = testSubSystems[GetData<uint32_t>() % testSubSystems.size()].c_str(),
        .switchName = testSwitchNames[GetData<uint32_t>() % testSwitchNames.size()].c_str(),
        .action = testActions[GetData<uint32_t>() % testActions.size()].c_str(),
        .switchState = testSwitchStates[GetData<uint32_t>() % testSwitchStates.size()].c_str()
    };
    bool isNull = GetData<bool>();
    if (isNull) {
...
void AudioSocketThreadAudioPnpUeventParseFuzzTest()
{
    ...
    const char *msg = testMsgs[GetData<uint32_t>() % testMsgs.size()].c_str();
    ssize_t strLength;
    bool isInvalid = GetData<bool>();
    if (isInvalid) {
...
void AudioSocketThreadDetectDPStateFuzzTest()
{
    AudioSocketThread audioSocketThread;
    AudioEvent audioEvent;
    audioEvent.eventType = GetData<uint32_t>();
