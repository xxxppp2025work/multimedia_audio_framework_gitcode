...
void AudioPolicyServiceEnhanceTwoFuzzTest()
{
    ...
    DeviceType deviceType = GetData<DeviceType>();
    GetServerPtr()->audioPolicyService_.HandleActiveDevice(deviceType);

    std::string networkId = "LocalDevice";
    GetServerPtr()->audioPolicyService_.ActivateNewDevice(networkId, deviceType, true);
    ...
    GetServerPtr()->audioPolicyService_.UpdateA2dpOffloadFlagForAllStream(sessionIDToSpatialization, deviceType);
...
void AudioPolicyServiceEnhanceFiveFuzzTest()
{
    ...
    std::vector<SourceType> sourceVec = {
    ...
    uint32_t sourceInt = GetData<uint32_t>() % sourceVec.size();
    SourceType source = sourceVec[sourceInt];
    GetServerPtr()->audioPolicyService_.
        GetTargetSourceTypeAndMatchingFlag(source, targetSource, useMatchingPropInfo);
    ...
    PipeInfo pipeInfo;
    GetServerPtr()->audioPolicyService_.GetHalNameForDevice(role, deviceType);
    GetServerPtr()->audioPolicyService_.GetPipeInfoByDeviceTypeForEc(role, deviceType, pipeInfo);
    ...
    uint32_t deviceTypeInt = GetData<uint32_t>() % deviceTypeVec.size();
    DeviceType deviceTypeEc = deviceTypeVec[deviceTypeInt];
    GetServerPtr()->audioPolicyService_.GetPipeNameByDeviceForEc(role, deviceTypeEc);
    ...
    GetServerPtr()->audioPolicyService_.UpdateAudioEcInfo(deviceType, deviceType);
}
...
void AudioPolicyServiceEnhanceSixFuzzTest()
{
    ...
    GetServerPtr()->audioPolicyService_.UpdateModuleInfoForMicRef(moduleInfo, source);
    GetServerPtr()->audioPolicyService_.UpdateStreamEcInfo(moduleInfo, SOURCE_TYPE_VOICE_COMMUNICATION);
    GetServerPtr()->audioPolicyService_.UpdateStreamCommonInfo(moduleInfo, targetInfo, source);
    GetServerPtr()->audioPolicyService_.isEcFeatureEnable_ = true;
    GetServerPtr()->audioPolicyService_.UpdateStreamCommonInfo(moduleInfo, targetInfo, source);
...
void AudioPolicyServiceEnhanceEightFuzzTest()
{
    ...
    DeviceType deviceType = GetData<DeviceType>();
    GetServerPtr()->audioPolicyService_.OpenRemoteAudioDevice(networkId, deviceRole, deviceType, remote);
