...
const uint32_t SAMPLENUM_NEW = 5;
const size_t SIZE_FLOAT = 5;
const size_t SIZE_COUNT = 1;
...
void AudioCommonConverterFuzzTest(const uint8_t *rawData, size_t size)
{
    ...
    uint8_t buffer[4] = {0};
    buffer[0] = BUFFER_CONSTANT;
    uint32_t samplePerFrame = AUDIO_24BIT_LENGTH;
    size_t floatBufferSize = SIZE_FLOAT;
    std::vector<float> floatBuffer(floatBufferSize, FLOAT_BUFFER);
    float volume = FLOAT_VOLUME;
    AudioCommonConverter::ConvertBufferToFloat(buffer, samplePerFrame, floatBuffer, volume);
    uint32_t samplePerFrameNew = SAMPLENUM_NEW;
    AudioCommonConverter::ConvertBufferToFloat(buffer, samplePerFrameNew, floatBuffer, volume);

    int32_t dst32Num = 0;
    int32_t *dst32 = &dst32Num;
    int16_t dst16Num = 0;
    int16_t *dst16 = &dst16Num;
    size_t count = SIZE_COUNT;
    int32_t format8BIT = AUDIO_SAMPLE_FORMAT_8BIT;
    AudioCommonConverter::ConvertBufferTo32Bit(buffer, format8BIT, dst32, count, volume);
    AudioCommonConverter::ConvertBufferTo16Bit(buffer, format8BIT, dst16, count, volume);
    int32_t format16BIT = AUDIO_SAMPLE_FORMAT_16BIT;
    AudioCommonConverter::ConvertBufferTo32Bit(buffer, format16BIT, dst32, count, volume);
    AudioCommonConverter::ConvertBufferTo16Bit(buffer, format16BIT, dst16, count, volume);
    int32_t format24BIT = AUDIO_SAMPLE_FORMAT_24BIT;
    AudioCommonConverter::ConvertBufferTo32Bit(buffer, format24BIT, dst32, count, volume);
    AudioCommonConverter::ConvertBufferTo16Bit(buffer, format24BIT, dst16, count, volume);
    int32_t format32BIT = AUDIO_SAMPLE_FORMAT_32BIT;
    AudioCommonConverter::ConvertBufferTo32Bit(buffer, format32BIT, dst32, count, volume);
    AudioCommonConverter::ConvertBufferTo16Bit(buffer, format32BIT, dst16, count, volume);
    int32_t format32FBIT = AUDIO_SAMPLE_FORMAT_32F_BIT;
    AudioCommonConverter::ConvertBufferTo32Bit(buffer, format32FBIT, dst32, count, volume);
    AudioCommonConverter::ConvertBufferTo16Bit(buffer, format32FBIT, dst16, count, volume);
    int32_t format = FORMAT_COUNT;
    AudioCommonConverter::ConvertBufferTo32Bit(buffer, format, dst32, count, volume);
    AudioCommonConverter::ConvertBufferTo16Bit(buffer, format, dst16, count, volume);
}
