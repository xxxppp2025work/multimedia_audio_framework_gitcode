...
public:
    ...
    void GetSleAudioDeviceList(std::vector<AudioDeviceDescriptor> &devices) {}
    void GetSleVirtualAudioDeviceList(std::vector<AudioDeviceDescriptor> &devices) {}
    bool IsInBandRingOpen(const std::string &device)
    {
        return false;
    }
    uint32_t GetSupportStreamType(const std::string &device)
    {
        return 0;
    }
    int32_t SetActiveSinkDevice(const std::string &device, uint32_t streamType)
    {
        return 0;
    }
    ...
    int32_t StopPlaying(const std::string &device, uint32_t streamType)
    {
        return 0;
    }
    int32_t ConnectAllowedProfiles(const std::string &remoteAddr)
    {
        return 0;
    }
    ...
    int32_t SendUserSelection(const std::string &device, uint32_t streamType)
    {
        return 0;
    }
};
...
void SleAudioDeviceManagerSetNearlinkDeviceMuteFuzzTest()
{
    ...
    manager.deviceVolumeConfigInfo_[device] = pairConfigInfo;
    AudioStreamType streamType = g_testAudioStreamTypes[GetData<uint32_t>() % g_testAudioStreamTypes.size()];
