...
void GetEcSamplingRateFuzzTest()
{
    ...
    uint32_t deviceListCount = GetData<uint32_t>() % deviceList.size();
    ...
    outModuleInfo->sampleRate_ = GetData<uint32_t>();
...
void GetEcChannelsFuzzTest()
{
    ...
    std::vector<std::string> insertList = {"", to_string(GetData<uint32_t>())};
