...
public:
    AudioCapturerImpl();
    explicit AudioCapturerImpl(std::unique_ptr<AudioCapturerImpl> obj);
    ~AudioCapturerImpl();

    static std::unique_ptr<AudioCapturerImpl> CreateAudioCapturerNativeObject();
    ...
    void StopSync();
    void ReleaseSync();
    taihe::array<uint8_t> ReadSync(int64_t size, bool isBlockingRead);
