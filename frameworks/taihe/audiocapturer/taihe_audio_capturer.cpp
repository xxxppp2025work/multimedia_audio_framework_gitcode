...
AudioCapturerImpl::AudioCapturerImpl(std::unique_ptr<AudioCapturerImpl> obj)
{
    if (obj != nullptr) {
...
std::unique_ptr<AudioCapturerImpl> AudioCapturerImpl::CreateAudioCapturerNativeObject()
{
    std::unique_ptr<AudioCapturerImpl> audioCapturerImpl = std::make_unique<AudioCapturerImpl>();
...
AudioCapturer AudioCapturerImpl::CreateAudioCapturerWrapper(OHOS::AudioStandard::AudioCapturerOptions capturerOptions)
{
    ...
    if (sCapturerOptions_ == nullptr) {
        AUDIO_ERR_LOG("sCapturerOptions create failed");
        return make_holder<AudioCapturerImpl, AudioCapturer>(nullptr);
    }
    *sCapturerOptions_ = capturerOptions;
    std::unique_ptr<AudioCapturerImpl> impl = AudioCapturerImpl::CreateAudioCapturerNativeObject();
    if (impl == nullptr) {
        AUDIO_ERR_LOG("failed to CreateAudioCapturerNativeObject");
        return make_holder<AudioCapturerImpl, AudioCapturer>(nullptr);
    }
    return make_holder<AudioCapturerImpl, AudioCapturer>(std::move(impl));
}
...
taihe::array<uint8_t> AudioCapturerImpl::ReadSync(int64_t size, bool isBlockingRead)
{
    if (audioCapturer_ == nullptr) {
        TaiheAudioError::ThrowErrorAndReturn(TAIHE_ERR_ILLEGAL_STATE, "audioCapturer_ is nullptr");
        return taihe::array<uint8_t>(DEFAULT_ARRAY_SIZE);
    }
    if (size <= 0) {
        TaiheAudioError::ThrowErrorAndReturn(TAIHE_ERR_INPUT_INVALID, "ReadSync size is invalid");
        return taihe::array<uint8_t>(DEFAULT_ARRAY_SIZE);
    }

    uint64_t userSize = static_cast<uint64_t>(size);
    uint8_t *buffer = new uint8_t[userSize];
    CHECK_AND_RETURN_RET_LOG(buffer != nullptr, taihe::array<uint8_t>(0), "buffer malloc failed, no memery");
    int64_t bytesRead = 0;
    while (static_cast<uint64_t>(bytesRead) < userSize) {
        int32_t len = audioCapturer_->Read(*(buffer + bytesRead),
            userSize - bytesRead, isBlockingRead);
        if (len >= 0) {
            bytesRead += len;
        } else {
            bytesRead = len;
    ...
    taihe::array<uint8_t> arrayBuffer = TaiheParamUtils::ToTaiheArrayBuffer(buffer, bytesRead);
    if (arrayBuffer.empty()) {
        AUDIO_ERR_LOG("ReadSync size is empty");
        return taihe::array<uint8_t>(DEFAULT_ARRAY_SIZE);
    }
    return arrayBuffer;
}
...
void AudioCapturerImpl::OffPeriodReach(optional_view<callback<void(double)>> callback)
{
    CHECK_AND_RETURN_RET_LOG(audioCapturer_ != nullptr, TaiheAudioError::ThrowErrorAndReturn(
        TAIHE_ERROR_INVALID_PARAM), "audioCapturer_ is nullptr");
    std::shared_ptr<uintptr_t> cacheCallback;
...
void AudioCapturerImpl::OffMarkReach(optional_view<callback<void(double)>> callback)
{
    CHECK_AND_RETURN_RET_LOG(audioCapturer_ != nullptr, TaiheAudioError::ThrowErrorAndReturn(
        TAIHE_ERROR_INVALID_PARAM), "audioCapturer_ is nullptr");
    std::shared_ptr<uintptr_t> cacheCallback;
