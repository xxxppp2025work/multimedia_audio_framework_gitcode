...
napi_threadsafe_function amRouDevChg_tsfn_ = nullptr;
...
void NapiAudioRountingAvailableDeviceChangeCallback::SafeJsCallbackAvailbleDeviceChangeWork(napi_env env, napi_value js_cb, void* context, void* data)
{
    ...
    std::shared_ptr<AudioRountingJsCallback> safeContext(
        ...
        [](AudioRountingJsCallback* ptr) {
            napi_release_threadsafe_function(amRouDevChg_tsfn_, napi_tsfn_abort);
            delete ptr;
    });
...
void NapiAudioRountingAvailableDeviceChangeCallback::AvailbleDeviceChangeTsfnFinalize(napi_env env, void *data, void *hint)
{
    ...
    std::unique_ptr<AudioRountingJsCallback> &jsCb)
{
    ...
    napi_create_threadsafe_function(event->callback->env_, nullptr, nullptr, cbName, 0, 1, event, AvailbleDeviceChangeTsfnFinalize, nullptr, SafeJsCallbackAvailbleDeviceChangeWork, &amRouDevChg_tsfn_);
    
    std::thread safeCallThread([event]() {
        AUDIO_INFO_LOG("OnJsCallbackAvailbleDeviceChange: safe thread start.");
        napi_acquire_threadsafe_function(amRouDevChg_tsfn_);
        napi_call_threadsafe_function(amRouDevChg_tsfn_, event, napi_tsfn_blocking);
    });
