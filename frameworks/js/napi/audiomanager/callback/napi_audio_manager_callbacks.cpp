...
void NapiAudioManagerCallback::SafeJsCallbackDeviceChangeWork(napi_env env, napi_value js_cb, void* context, void* data)
{
    ...
    std::shared_ptr<AudioManagerJsCallback> safeContext(
        ...
        [](AudioManagerJsCallback* ptr) {
            napi_release_threadsafe_function(amDevChg_tsfn_, napi_tsfn_abort);
            delete ptr;
    });
    ...
    AUDIO_INFO_LOG("SafeJsCallbackDeviceChangeWork: safe device change callback working.");
...
void NapiAudioManagerCallback::OnJsCallbackDeviceChange(std::unique_ptr<AudioManagerJsCallback> &jsCb)
{
    ...
    napi_create_threadsafe_function(event->callback->env_, nullptr, nullptr, cbName, 0, 1, event,
        DeviceChangeTsfnFinalize, nullptr, SafeJsCallbackDeviceChangeWork, &amDevChg_tsfn_);
    
    std::thread safeCallThread([event]() {
        AUDIO_INFO_LOG("OnJsCallbackRingerMode: safe thread start.");
        napi_acquire_threadsafe_function(amDevChg_tsfn_);
        napi_call_threadsafe_function(amDevChg_tsfn_, event, napi_tsfn_blocking);
    });
    ...
    napi_env env, napi_value js_cb, void* context, void* data)
{
    ...
    std::shared_ptr<AudioManagerJsCallback> safeContext(
        static_cast<AudioManagerJsCallback*>(data),
        [](AudioManagerJsCallback* ptr) {
            napi_release_threadsafe_function(amMicBlocked_tsfn_, napi_tsfn_abort);
            delete ptr;
    });
    AudioManagerJsCallback *event = reinterpret_cast<AudioManagerJsCallback *>(data);
    CHECK_AND_RETURN_LOG(event != nullptr, "event is nullptr");
...
void NapiAudioManagerCallback::OnJsCallbackMicrophoneBlocked(std::unique_ptr<AudioManagerJsCallback> &jsCb)
{
    ...
    napi_create_string_utf8(event->callback->env_, event->callbackName.c_str(), event->callbackName.length(), &cbName);
    napi_create_threadsafe_function(event->callback->env_, nullptr, nullptr, cbName, 0, 1, event,
        MicrophoneBlockedTsfnFinalize, nullptr, SafeJsCallbackMicrophoneBlockedWork, &amMicBlocked_tsfn_);
    
    std::thread safeCallThread([event]() {
        AUDIO_INFO_LOG("OnJsCallbackMicrophoneBlocked: safe thread start.");
        napi_acquire_threadsafe_function(amMicBlocked_tsfn_);
        napi_call_threadsafe_function(amMicBlocked_tsfn_, event, napi_tsfn_blocking);
    });
