...
napi_threadsafe_function MicStateChg_tsfn_ = nullptr;
...
void NapiAudioManagerMicStateChangeCallback::SafeJsCallbackMicStateChangeWork(napi_env env, napi_value js_cb, void* context, void* data)
{
    ...
    std::shared_ptr<AudioManagerMicStateChangeJsCallback> safeContext(
        ...
        [](AudioManagerMicStateChangeJsCallback* ptr) {
            napi_release_threadsafe_function(MicStateChg_tsfn_, napi_tsfn_abort);
            delete ptr;
    });
    ...
    (std::unique_ptr<AudioManagerMicStateChangeJsCallback> &jsCb)
{
    ...
    napi_create_threadsafe_function(event->callback->env_, nullptr, nullptr, cbName, 0, 1, event, MicStateChangeTsfnFinalize, nullptr, SafeJsCallbackMicStateChangeWork, &MicStateChg_tsfn_);
    
    std::thread safeCallThread([event]() {
        AUDIO_INFO_LOG("OnJsCallbackMicStateChange: safe thread start.");
        napi_acquire_threadsafe_function(MicStateChg_tsfn_);
        napi_call_threadsafe_function(MicStateChg_tsfn_, event, napi_tsfn_blocking);
    });
