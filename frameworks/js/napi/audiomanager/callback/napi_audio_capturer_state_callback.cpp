...
napi_threadsafe_function amacState_tsfn_ = nullptr;
...
void NapiAudioCapturerStateCallback::SafeJsCallbackCapturerStateWork(napi_env env, napi_value js_cb, void* context, void* data)
{
    ...
    std::shared_ptr<AudioCapturerStateJsCallback> safeContext(
        ...
        [](AudioCapturerStateJsCallback* ptr) {
            napi_release_threadsafe_function(amacState_tsfn_, napi_tsfn_abort);
            delete ptr;
    });
    ...
    AUDIO_INFO_LOG("SafeJsCallbackCapturerStateWork: safe capture state callback working.");
    do {
...
void NapiAudioCapturerStateCallback::OnJsCallbackCapturerState(std::unique_ptr<AudioCapturerStateJsCallback> &jsCb)
{
    ...
    napi_create_threadsafe_function(event->callback->env_, nullptr, nullptr, cbName, 0, 1, event, CapturerStateTsfnFinalize, nullptr, SafeJsCallbackCapturerStateWork, &amacState_tsfn_);
    
    std::thread safeCallThread([event]() {
        AUDIO_INFO_LOG("OnJsCallbackCapturerState: safe thread start.");
        napi_acquire_threadsafe_function(amacState_tsfn_);
        napi_call_threadsafe_function(amacState_tsfn_, event, napi_tsfn_blocking);
    });
