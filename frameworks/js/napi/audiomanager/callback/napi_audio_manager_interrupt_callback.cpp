...
napi_threadsafe_function amInterrupt_tsfn_ = nullptr;
...
void NapiAudioManagerInterruptCallback::SafeJsCallbackAudioManagerInterruptWork(napi_env env, napi_value js_cb, void* context, void* data)
{
    ...
    std::shared_ptr<AudioManagerInterruptJsCallback> safeContext(
        ...
        [](AudioManagerInterruptJsCallback* ptr) {
            napi_release_threadsafe_function(amInterrupt_tsfn_, napi_tsfn_abort);
            delete ptr;
    });
    ...
    std::unique_ptr<AudioManagerInterruptJsCallback> &jsCb)
{
    ...
    napi_create_threadsafe_function(event->callback->env_, nullptr, nullptr, cbName, 0, 1, event, AudioManagerInterruptTsfnFinalize, nullptr, SafeJsCallbackAudioManagerInterruptWork, &amInterrupt_tsfn_);
    
    std::thread safeCallThread([event]() {
        AUDIO_INFO_LOG("OnJsCallbackAudioManagerInterrupt: safe thread start.");
        napi_acquire_threadsafe_function(amInterrupt_tsfn_);
        napi_call_threadsafe_function(amInterrupt_tsfn_, event, napi_tsfn_blocking);
    });
