...
napi_threadsafe_function amVolEnt_tsfn_ = nullptr;
...
void NapiAudioVolumeKeyEvent::SafeJsCallbackVolumeEventWork(napi_env env, napi_value js_cb, void* context, void* data)
{
    ...
    std::shared_ptr<AudioVolumeKeyEventJsCallback> safeContext(
        static_cast<AudioVolumeKeyEventJsCallback*>(data),
        [](AudioVolumeKeyEventJsCallback* ptr) {
            napi_release_threadsafe_function(amVolEnt_tsfn_, napi_tsfn_abort);
            delete ptr;
    });
...
void NapiAudioVolumeKeyEvent::OnJsCallbackVolumeEvent(std::unique_ptr<AudioVolumeKeyEventJsCallback> &jsCb)
{
    ...
    napi_create_threadsafe_function(event->callback->env_, nullptr, nullptr, cbName, 0, 1, event, VolumeEventTsfnFinalize, nullptr, SafeJsCallbackVolumeEventWork, &amVolEnt_tsfn_);
    
    std::thread safeCallThread([event]() {
        AUDIO_INFO_LOG("OnJsCallbackVolumeEvent: safe thread start.");
        napi_acquire_threadsafe_function(amVolEnt_tsfn_);
        napi_call_threadsafe_function(amVolEnt_tsfn_, event, napi_tsfn_blocking);
    });
