...
napi_threadsafe_function amRmChg_tsfn_ = nullptr;
...
void NapiAudioRingerModeCallback::SafeJsCallbackRingModeWork(napi_env env, napi_value js_cb, void* context, void* data)
{
    ...
    std::shared_ptr<AudioRingerModeJsCallback> safeContext(
        ...
        [](AudioRingerModeJsCallback* ptr) {
            napi_release_threadsafe_function(amRmChg_tsfn_, napi_tsfn_abort);
            delete ptr;
    });
    ...
    AUDIO_INFO_LOG("SafeJsCallbackInterruptWork: safe ringer mode callback working.");
    do {
...
void NapiAudioRingerModeCallback::OnJsCallbackRingerMode(std::unique_ptr<AudioRingerModeJsCallback> &jsCb)
{
    ...
    napi_create_threadsafe_function(event->callback->env_, nullptr, nullptr, cbName, 0, 1, event, RingModeTsfnFinalize, nullptr, SafeJsCallbackRingModeWork, &amRmChg_tsfn_);
    
    std::thread safeCallThread([event]() {
        AUDIO_INFO_LOG("OnJsCallbackRingerMode: safe thread start.");
        napi_acquire_threadsafe_function(amRmChg_tsfn_);
        napi_call_threadsafe_function(amRmChg_tsfn_, event, napi_tsfn_blocking);
    });
