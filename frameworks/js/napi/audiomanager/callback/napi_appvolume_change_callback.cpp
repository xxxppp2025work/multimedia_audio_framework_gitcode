...
void NapiAudioManagerAppVolumeChangeCallback::RemoveAllAudioVolumeChangeForUidCbRef(int32_t appUid)
{
    std::lock_guard<std::mutex> lock(mutex_);
    for (auto iter = appVolumeChangeForUidList_.begin(); iter != appVolumeChangeForUidList_.end();) {
        if (iter->second == appUid) {
            AUDIO_INFO_LOG("RemoveAllAudioVolumeChangeForUidCbRef: find appUid, erase it");
...
void NapiAudioManagerAppVolumeChangeCallback::RemoveAudioVolumeChangeForUidCbRef(api_env env,
    napi_value callback, int32_t appUid)
{
    std::lock_guard<std::mutex> lock(mutex_);
    for (auto iter = appVolumeChangeForUidList_.begin(); iter != appVolumeChangeForUidList_.end();) {
        if (IsSameCallback(env, callback, iter->first->cb_) && iter->second == appUid) {
...
void NapiAudioManagerAppVolumeChangeCallback::RemoveSelfAudioVolumeChangeCbRef(api_env env,
    napi_value callback)
{
    std::lock_guard<std::mutex> lock(mutex_);
