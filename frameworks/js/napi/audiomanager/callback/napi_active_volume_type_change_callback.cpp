...
NapiAudioManagerActiveVolumeTypeChangeCallback::NapiAudioManagerActiveVolumeTypeChangeCallback(napi_env env)
    : env_(env)
{
    AUDIO_DEBUG_LOG("NapiAudioManagerActiveVolumeTypeChangeCallback: instance create");
}
...
NapiAudioManagerActiveVolumeTypeChangeCallback::~NapiAudioManagerActiveVolumeTypeChangeCallback()
{
    ...
    AUDIO_DEBUG_LOG("NapiAudioManagerActiveVolumeTypeChangeCallback: instance destroy");
}
...
void NapiAudioManagerActiveVolumeTypeChangeCallback::ActiveVolumeTypeChangeTsfnFinalize(napi_env env, void *data,
    void *hint)
{
    AUDIO_INFO_LOG("NapiAudioManagerActiveVolumeTypeChangeCallback: safe thread resource release.");
}
    ...
    napi_env env, napi_value js_cb, void *context, void *data)
{
    ...
    CHECK_AND_RETURN_LOG((event != nullptr) && (event->callback != nullptr),
        "NapiAudioManagerActiveVolumeTypeChangeCallback: no memory");
    ...
    AUDIO_INFO_LOG("NapiAudioManagerActiveVolumeTypeChangeCallback: safe js callback working.");
    ...
    do {
        ...
        CHECK_AND_BREAK_LOG(nstatus == napi_ok && args[0] != nullptr,
            "%{public}s fail to create active volume type change callback", request.c_str());
...
void NapiAudioManagerActiveVolumeTypeChangeCallback::OnActiveVolumeTypeChanged(const AudioVolumeType &event)
{
    ...
    AUDIO_DEBUG_LOG("OnActiveVolumeTypeChanged enter");
...
{
    if (jsCb.get() == nullptr) {
        AUDIO_ERR_LOG("OnJsCallbackActiveVolumeTypeChange: jsCb.get() is null");
        return;
    }
    ...
    const std::string &callbackName, napi_value args)
{
    ...
    CHECK_AND_RETURN_LOG(status == napi_ok && callback != nullptr,
        "NapiAudioManagerActiveVolumeTypeChangeCallback: creating reference for callback fail");
    std::shared_ptr<AutoRef> cb = std::make_shared<AutoRef>(env_, callback);
    if (callbackName == ACTIVE_VOLUME_TYPE_CHANGE_CALLBACK_NAME) {
        activeVolumeTypeChangeList_.push_back(cb);
    }  else {
        AUDIO_ERR_LOG("NapiAudioManagerActiveVolumeTypeChangeCallback: Unknown callback type: %{public}s",
            callbackName.c_str());
    }
}
    ...
    napi_value callback)
{
    ...
    for (auto iter = activeVolumeTypeChangeList_.begin(); iter != activeVolumeTypeChangeList_.end();) {
        if (IsSameCallback(env, callback, (*iter)->cb_)) {
            AUDIO_INFO_LOG("RemoveActiveVolumeTypeChangeCbRef: find js callback, erase it");
            activeVolumeTypeChangeList_.erase(iter++);
        } else {
            iter++;
        }
    }
    AUDIO_INFO_LOG("RemoveActiveVolumeTypeChangeCbRef: remove callback finish");
}
...
void NapiAudioManagerActiveVolumeTypeChangeCallback::RemoveAllActiveVolumeTypeChangeCbRef()
{
    ...
    AUDIO_INFO_LOG("RemoveAllActiveVolumeTypeChangeCbRef: remove callback finish");
}
