...
{   
    if (napiRoutingMgr->microphoneBlockedCallbackNapi_ != nullptr) {
        ...
        if (callback == nullptr || cb->GetRoutingMicrophoneBlockedCbListSize() == 0) {
            int32_t ret = napiRoutingMgr->audioMngr_->UnsetMicrophoneBlockedCallback(DeviceFlag::ALL_L_D_DEVICES_FLAG,
                napiRoutingMgr->microphoneBlockedCallbackNapi_);
            CHECK_AND_RETURN_LOG(ret == SUCCESS, "UnRegisterMicrophoneBlockedCallback Failed");
            napiRoutingMgr->deviceChangeCallbackNapi_.reset();
            napiRoutingMgr->deviceChangeCallbackNapi_ = nullptr;
            return;
        }
        ...
        AUDIO_ERR_LOG("UnRegisterMicrophoneBlockedCallback: UnRegisterMicrophoneBlockedCallback is null");
    }
}
