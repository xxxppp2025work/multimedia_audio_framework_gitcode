    ...
    napi_value *args, const std::string &cbName)
{
    ...
    if (!cbName.compare(RENDERERCHANGE_CALLBACK_NAME)) {
        ...
        RegisterRendererErrorCallback(env, args, cbName, napiStreamMgr);
    } else {
...
void NapiAudioStreamMgr::RegisterRendererErrorCallback(napi_env env, napi_value *args,
    const std::string &cbName, NapiAudioStreamMgr *napiStreamMgr)
{
    if (!napiStreamMgr->errorCallbackNapi_) {
        ...
        int32_t ret =
            napiStreamMgr->audioStreamMngr_->SetAudioRendererErrorCallback(napiStreamMgr->errorCallbackNapi_);
        CHECK_AND_RETURN_LOG(ret == SUCCESS,
    ...
    size_t argc, napi_value *args, const std::string &cbName)
{
    ...
    if (!cbName.compare(RENDERERCHANGE_CALLBACK_NAME)) {
        ...
        UnregisterRendererErrorCallback(napiStreamMgr, argc, args);
...
void NapiAudioStreamMgr::UnregisterRendererErrorCallback(NapiAudioStreamMgr *napiStreamMgr,
    size_t argc, napi_value *args)
{
    CHECK_AND_RETURN_LOG(napiStreamMgr->errorCallbackNapi_ != nullptr,
    ...
    int32_t ret = napiStreamMgr->audioStreamMngr_->UnsetAudioRendererErrorCallback();
