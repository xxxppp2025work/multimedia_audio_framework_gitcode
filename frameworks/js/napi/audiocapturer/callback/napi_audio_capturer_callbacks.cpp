...
void NapiAudioCapturerCallback::OnJsCallbackInterrupt(std::unique_ptr<AudioCapturerJsCallback> &jsCb)
{
    ...
    CHECK_AND_RETURN_LOG(event != nullptr, "OnJsCallbackInterrupt: event is nullptr.");
    ...
    napi_create_threadsafe_function(event->callback->env_, nullptr, nullptr, cbName, 0, 1, event,
        InterruptTsfnFinalize, nullptr, SafeJsCallbackInterruptWork, &acInterrupt_tsfn_);
    
    std::thread safeCallThread([event]() {
        AUDIO_INFO_LOG("OnJsCallbackInterrupt: safe thread start.");
        napi_acquire_threadsafe_function(acInterrupt_tsfn_);
        napi_call_threadsafe_function(acInterrupt_tsfn_, event, napi_tsfn_blocking);
    });
...
void NapiAudioCapturerCallback::OnJsCallbackStateChange(std::unique_ptr<AudioCapturerJsCallback> &jsCb)
{
    ...
    napi_create_string_utf8(event->callback->env_, event->callbackName.c_str(), event->callbackName.length(), &cbName);
    napi_create_threadsafe_function(event->callback->env_, nullptr, nullptr, cbName, 0, 1, event,
        StateChangeTsfnFinalize, nullptr, SafeJsCallbackStateChangeWork, &acStateChg_tsfn_);
