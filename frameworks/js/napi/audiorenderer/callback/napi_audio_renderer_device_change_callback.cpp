...
napi_threadsafe_function arOutputDevChg_tsfn_ = nullptr;
napi_threadsafe_function arDevInfo_tsfn_ = nullptr;
...
void NapiAudioRendererDeviceChangeCallback::SafeJsCallbackRendererDeviceInfoWork(napi_env env, napi_value js_cb, void* context, void* data)
{
    ...
    std::shared_ptr<AudioRendererDeviceChangeJsCallback> safeContext(
        ...
        [](AudioRendererDeviceChangeJsCallback* ptr) {
            napi_release_threadsafe_function(arDevInfo_tsfn_, napi_tsfn_abort);
            delete ptr;
    });
    ...
    const DeviceInfo &deviceInfo)
{
    ...
    napi_create_threadsafe_function(event->env_, nullptr, nullptr, cbName, 0, 1, event, RendererDeviceInfoTsfnFinalize, nullptr, SafeJsCallbackRendererDeviceInfoWork, &arDevInfo_tsfn_);
    
    std::thread safeCallThread([event]() {
        AUDIO_INFO_LOG("OnJsCallbackRendererDeviceInfo: safe thread start.");
        napi_acquire_threadsafe_function(arDevInfo_tsfn_);
        napi_call_threadsafe_function(arDevInfo_tsfn_, event, napi_tsfn_blocking);
    });
...
void NapiAudioRendererOutputDeviceChangeWithInfoCallback::SafeJsCallbackOutputDeviceInfoWork(napi_env env, napi_value js_cb, void* context, void* data)
{
    CHECK_AND_RETURN_LOG(data != nullptr, "data is nullptr.");
    std::shared_ptr<AudioRendererOutputDeviceChangeWithInfoJsCallback> safeContext(
        ...
        [](AudioRendererOutputDeviceChangeWithInfoJsCallback* ptr) {
            napi_release_threadsafe_function(arOutputDevChg_tsfn_, napi_tsfn_abort);
            delete ptr;
    });
    AudioRendererOutputDeviceChangeWithInfoJsCallback *event = reinterpret_cast<AudioRendererOutputDeviceChangeWithInfoJsCallback *>(data);
    CHECK_AND_RETURN_LOG((event != nullptr) && (event->callback_ != nullptr),
...
void NapiAudioRendererOutputDeviceChangeWithInfoCallback::OutputDeviceInfoTsfnFinalize(napi_env env, void *data, void *hint)
{
    ...
    const DeviceInfo &deviceInfo, AudioStreamDeviceChangeReason reason)
{
    ...
    napi_create_string_utf8(event->env_, event->callbackName.c_str(), event->callbackName.length(), &cbName);
    napi_create_threadsafe_function(event->env_, nullptr, nullptr, cbName, 0, 1, event, OutputDeviceInfoTsfnFinalize, nullptr, SafeJsCallbackOutputDeviceInfoWork, &arOutputDevChg_tsfn_);
    
    std::thread safeCallThread([event]() {
        AUDIO_INFO_LOG("OnJsCallbackOutputDeviceInfo: safe thread start.");
        napi_acquire_threadsafe_function(arOutputDevChg_tsfn_);
        napi_call_threadsafe_function(arOutputDevChg_tsfn_, event, napi_tsfn_blocking);
    });
