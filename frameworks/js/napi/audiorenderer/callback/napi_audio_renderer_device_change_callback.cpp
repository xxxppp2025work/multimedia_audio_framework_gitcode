...
void NapiAudioRendererDeviceChangeCallback::SaveCallbackReference(napi_value args)
{
...
void NapiAudioRendererDeviceChangeCallback::RemoveCallbackReference(napi_env env,
...
void NapiAudioRendererOutputDeviceChangeWithInfoCallback::SaveCallbackReference(napi_value args)
{
    std::lock_guard<std::mutex> lock(mutex_);
    ...
    std::shared_ptr<AutoRef> cb = std::make_shared<AutoRef>(env_, callback);
    callbacks_.push_back(cb);
    AUDIO_INFO_LOG("NapiAudioRendererOutputDeviceChangeWithInfoCallback SaveCallbackReference success");
}
...
void NapiAudioRendererOutputDeviceChangeWithInfoCallback::RemoveCallbackReference(napi_env env,
    napi_value callback, napi_value args)
{
    std::lock_guard<std::mutex> lock(mutex_);
    bool isEquals = false;
