...
napi_threadsafe_function arPos_tsfn_ = nullptr;
...
void NapiRendererPositionCallback::SafeJsCallbackPositionWork(napi_env env, napi_value js_cb, void* context, void* data)
{
    ...
    std::shared_ptr<RendererPositionJsCallback> safeContext(
        ...
        [](RendererPositionJsCallback* ptr) {
            napi_release_threadsafe_function(arPos_tsfn_, napi_tsfn_abort);
            delete ptr;
    });
...
void NapiRendererPositionCallback::OnJsRendererPositionCallback(std::unique_ptr<RendererPositionJsCallback> &jsCb)
{
    ...
    napi_create_threadsafe_function(event->callback->env_, nullptr, nullptr, cbName, 0, 1, event, PositionTsfnFinalize, nullptr, SafeJsCallbackPositionWork, &arPos_tsfn_);
    
    std::thread safeCallThread([event]() {
        AUDIO_INFO_LOG("OnJsCallbackRingerMode: safe thread start.");
        napi_acquire_threadsafe_function(arPos_tsfn_);
        napi_call_threadsafe_function(arPos_tsfn_, event, napi_tsfn_blocking);
    });
