...
napi_threadsafe_function arDataReq_tsfn_ = nullptr;
...
void NapiRendererDataRequestCallback::SafeJsCallbackDataRequestWork(napi_env env, napi_value js_cb, void* context, void* data)
{
    ...
    std::shared_ptr<RendererDataRequestJsCallback> safeContext(
        ...
        [](RendererDataRequestJsCallback* ptr) {
            napi_release_threadsafe_function(arDataReq_tsfn_, napi_tsfn_abort);
            delete ptr;
    });
    ...
    std::unique_ptr<RendererDataRequestJsCallback> &jsCb)
{
    ...
    napi_create_threadsafe_function(event->callback->env_, nullptr, nullptr, cbName, 0, 1, event, DataRequestTsfnFinalize, nullptr, SafeJsCallbackDataRequestWork, &arDataReq_tsfn_);
    
    std::thread safeCallThread([event]() {
        AUDIO_INFO_LOG("OnJsRendererDataRequestCallback: safe thread start.");
        napi_acquire_threadsafe_function(arDataReq_tsfn_);
        napi_call_threadsafe_function(arDataReq_tsfn_, event, napi_tsfn_blocking);
    });
