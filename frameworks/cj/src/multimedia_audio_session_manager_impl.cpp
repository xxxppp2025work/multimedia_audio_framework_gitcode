...
{
    if (type != AUDIO_SESSION_CALLBACK_NAME) {
        AUDIO_ERR_LOG("[SessionManager::On] Type error!");
        return;
    }
    ...
    if (!sessionCallback_) {
        ...
        if (!sessionCallback_) {
            AUDIO_ERR_LOG("[SessionManager::On] Memory allocation failed!");
            return;
        }
    }
    ...
    if (ret != SUCCESS_CODE) {
        AUDIO_ERR_LOG("[SessionManager::On] SetAudioSessionCallback failed!");
        *errorCode = ret;
        return;
    }
