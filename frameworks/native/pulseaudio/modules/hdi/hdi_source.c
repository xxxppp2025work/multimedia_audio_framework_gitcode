...
static void PrepareEcCapture(struct Userdata *u)
{
    ...
    if (u->ecType == EC_DIFFERENT_ADAPTER) {
        ...
        u->sourceAdapterEc = GetSourceAdapter(DEFAULT_DEVICE_CLASS, -1, HDI_ADAPTER_ID_INFO_EC);
        if (u->sourceAdapterEc == NULL) {
...
static void PrepareMicRefCapture(struct Userdata *u)
{
    ...
    u->sourceAdapterMicRef = GetSourceAdapter(DEFAULT_DEVICE_CLASS, -1, HDI_ADAPTER_ID_INFO_MIC_REF);
    if (u->sourceAdapterMicRef == NULL) {
...
pa_source *PaHdiSourceNew(pa_module *m, pa_modargs *ma, const char *driver)
{
    ...
    if (u->attrs.sourceType == SOURCE_TYPE_WAKEUP) {
        u->sourceAdapter = GetSourceAdapter(deviceClass, u->attrs.sourceType,
