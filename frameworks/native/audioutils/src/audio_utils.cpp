...
bool PermissionUtil::NotifyStart(uint32_t targetTokenId, uint32_t sessionId)
{
    ...
    if (g_tokenIdRecordMap.count(targetTokenId)) {
        ...
        AUDIO_WARNING_LOG("StartUsingPermission for tokenId:%{public}u sessionId:%{public}u",
            targetTokenId, sessionId);
        ...
        AUDIO_INFO_LOG("AddPermissionUsedRecord for tokenId:%{public}u sessionId:%{public}u!",
            targetTokenId, sessionId);
...
bool PermissionUtil::ReNotifyStart(uint32_t targetTokenId, int32_t &res)
{
    ...
    WatchTimeout guardStop("Security::AccessToken::PrivacyKit::StopUsingPermission:ReNotifyStart");
    ...
    CHECK_AND_RETURN_RET_LOG(stopRet == 0, false, "StopUsingPermission for tokenId:%{public}u failed!"
    ...
    WatchTimeout guardStart("Security::AccessToken::PrivacyKit::StartUsingPermission:ReNotifyPrivacy");
    ...
    CHECK_AND_RETURN_RET_LOG(res != Security::AccessToken::ERR_PERMISSION_ALREADY_START_USING, false,
        "The PrivacyKit return ERR_PERMISSION_ALREADY_START_USING again!");
    return true;
}
...
bool PermissionUtil::NotifyStop(uint32_t targetTokenId, uint32_t sessionId)
{
    ...
    if (g_tokenIdRecordMap[targetTokenId].empty()) {
        ...
        AUDIO_WARNING_LOG("StopUsingPermission for tokenId:%{public}u sessionId:%{public}u",
            targetTokenId, sessionId);
        ...
        guard.CheckCurrTimeout();
        CHECK_AND_RETURN_RET_LOG(res == 0, false, "StopUsingPermission for tokenId:%{public}u failed!,"
            "The PrivacyKit error code:%{public}d", targetTokenId, res);
    }
    return true;
}
