...
struct XmlFuncHandle {
    ...
    bool (*xmlHasProp)(xmlNode *node, const xmlChar *propName);
    xmlChar *(*xmlGetProp)(xmlNode *node, const xmlChar *propName);
...
bool DlopenUtils::Init()
{
    if (g_refCount_.load() == 0) {
        ...
        xmlFuncHandle_->xmlReadFile = reinterpret_cast<decltype(xmlFuncHandle_->xmlReadFile)>(dlsym(libHandle, "xmlReadFile")); 
        xmlFuncHandle_->xmlDocGetRootElement = reinterpret_cast<decltype(xmlFuncHandle_->xmlDocGetRootElement)>(dlsym(libHandle, "xmlDocGetRootElement")); 
        xmlFuncHandle_->xmlHasProp = reinterpret_cast<decltype(xmlFuncHandle_->xmlHasProp)>(dlsym(libHandle, "xmlHasProp")); 
        xmlFuncHandle_->xmlGetProp = reinterpret_cast<decltype(xmlFuncHandle_->xmlGetProp)>(dlsym(libHandle, "xmlGetProp"));
        xmlFuncHandle_->xmlFreeDoc = reinterpret_cast<decltype(xmlFuncHandle_->xmlFreeDoc)>(dlsym(libHandle, "xmlFreeDoc"));
        xmlFuncHandle_->xmlFree = reinterpret_cast<decltype(xmlFuncHandle_->xmlFree)>(dlsym(libHandle, "xmlFree"));
        xmlFuncHandle_->xmlCleanupParser = reinterpret_cast<decltype(xmlFuncHandle_->xmlCleanupParser)>(dlsym(libHandle, "xmlCleanupParser"));
        xmlFuncHandle_->xmlStrcmp = reinterpret_cast<decltype(xmlFuncHandle_->xmlStrcmp)>(dlsym(libHandle, "xmlStrcmp"));
    }
...
std::unique_ptr<AudioXmlNode> AudioXmlNode::Create()
{
    return std::make_unique<AudioXmlNodeInner>();
}
...
{
    CHECK_AND_RETURN_RET_LOG(xmlFuncHandle_ != nullptr, ERROR, "xmlFuncHandle is nullptr!");
    CHECK_AND_RETURN_RET_LOG(xmlFuncHandle_->xmlHasProp(curNode_, reinterpret_cast<const xmlChar*>(propName)), ERROR,
        "curNode has no Prop: %{public}s", propName);

    xmlChar *tempValue = xmlFuncHandle_->xmlGetProp(curNode_, reinterpret_cast<const xmlChar*>(propName));
    if (tempValue != nullptr) {
        result = std::to_string(reinterpret_cast<char*>(tempValue));
...
{
    return curNode_->type == XML_ELEMENT_NODE && (StrcmpXml(curNode_->name, reinterpret_cast<const xmlChar*>(propName)) == 0);
}
