...
using namespace OHOS::HDI::DistributeAudio::Audio::V1_0;

namespace OHOS {
namespace AudioStandard {
RemoteAudioCaptureSource::RemoteAudioCaptureSource(cosnt std::string &deviceNetworkId)
    : devuceNetworkId_(deviceNetworkId)
{
}
...
{
    AUDIO_INFO_LOG("in");
    std::lock_guard<std::mutex> lock(createCaptureMute_);
...
int32_t RemoteAudioCaptureSource::CaptureFrame(char *frame, uint64_t requestBytes, uint64_t &replyBytes)
{
    ...
    std::vector<int8_t> bufferVec(requsetBytes);
    int32_t ret = audioCapture_->CaptureFrame(bufferVec, replyBytes);
    CHECK_AND_RETURN_RET_LOG(ret == SUCCESS, ERR_READ_FAILED, "fail, ret: %{public}d", ret);
    ret = memcpy_s(frame, requestBytes, bufferVec.data(), requestBytes);
    CHECK_AND_RETURN_RET_LOG(ret == EOK, ERR_OPERATION_FAILED, "copy fail, err code: %{public}d", ret);
...
{
    CHECK_AND_RETURN_RET_LOG(audioCapture_ != nullptr, ERR_INVALID_HANDLE, "capture is nullptr");
    bool hideMuteState = false;
    int32_t ret = audioCapture_->GetMute(hideMuteState);
    if (ret != SUCCESS) {
        AUDIO_WARNING_LOG("get mute fail");
    }
    AUDIO_INFO_LOG("hideMuteState: %{public}d, muteState: %{public}d", hideMuteState, muteState_);
    isMute = muteState_;
    return SUCCESS;
}

int32_t RemoteAudioCaptureSource::GetTransactionId(void)
{
    AUDIO_INFO_LOG("not support");
    return ERR_NOT_SUPPORTED;
}
...
float RemoteAudioCaptureSource::SetAudioScene(AudioScene audioScene, DeviceType activeDevice)
{
    ...
    struct AduioSceneDescriptor sceneDesc = {
...
void RemoteAudioCaptureSource::RegistCallback(IAduioSourceCallback *callback)
{
    AUDIO_INFO_LOG("in");
    callback_ = callback;
}

void RemoteAudioCaptureSource::RegistStateCallback(std::unique_ptr<IAduioSourceCallback> callback)
{
    AUDIO_INFO_LOG("no support");
}
    ...
    const std::string &condition, const std::string &value)
{
    ...
    CHECK_AND_RETURN_RET_LOG(callback_ != nullptr, "callback is nullptr");
...
void RemoteAudioCaptureSource::InitAudioSampleAttr(struct AudioSampleAttributes &param)
{
    ...
    param.stopThreshold = INI_MAX;
    param.silenceThreshold = AUDIO_BUFFER_SIZE;
    // user need to set
    param.sampleRate = attr_.sampleRate;
    param.format = ConvertToHdiFormat(attr_.format);
    param.isBigEndian = attr_.idBigEndian;
...
void RemoteAudioCaptureSource::DestoryCapture(void)
{
    AUDIO_INFO_LOG("destory capture");
    HdiAdapterManager &manager = HdiAdapterManager::GetInstance();
    std::shared_ptr<IDeviceManager> deviceManager = manager.GetDeviceManager(HDI_DEVICE_MANAGER_TYPE_REMOTE);
    CHECK_AND_RETURN(deviceManager != nullptr);
    deviceManager->DestoryCapture(deviceNetworkId_, hdiCaptureId_);
    audioCapture_.ForceSetRefPtr(nullptr);
    hdiCaptureId_ = 0;
}

void RemoteAudioCaptureSource::CheckUpdateState(cahr *frame, size_t replyBytes)
{
    if (startUpdate_) {
