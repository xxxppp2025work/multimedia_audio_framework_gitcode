...
void AudioEffectChain::SetDeviceSwitchState(const std::string &deviceSwitchState)
{
    CHECK_AND_RETURN_LOG(StringConverter(deviceSwitchState, deviceSwitchState_),
        "convert invalid deviceSwitchState: %{public}s", deviceSwitchState.c_str());
}
...
int32_t AudioEffectChain::SetEffectParamToHandle(AudioEffectHandle handle, int32_t &replyData)
{
    ...
    data[DEVICE_SWITCH_STATE_INDEX] = static_cast<int32_t>(deviceSwitchState_);
