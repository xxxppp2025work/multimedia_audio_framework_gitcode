...
public:
    ...
    virtual int32_t OnCapturerFilterChange(uint32_t streamId, const AudioPlaybackCaptureConfig &newConfig,
        int32_t innerCapId) = 0;

    // This will be called when a filter released.
    virtual int32_t OnCapturerFilterRemove(uint32_t streamId, int32_t innerCapId) = 0;
};
}  // namespace AudioStandard
}  // namespace OHOS
