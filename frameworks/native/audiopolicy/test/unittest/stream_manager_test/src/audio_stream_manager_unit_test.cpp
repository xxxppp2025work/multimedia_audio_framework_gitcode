    ...
    TestSize.Level1)
{
    ...
    for (int32_t i = 0; i < VALUE_HUNDRED; i++) {
        ...
        EXPECT_EQ(true, (*(*audioRendererChangeInfos[0]->outputDeviceInfo.audioStreamInfo_.rbegin()).
            samplingRate.rbegin() >= SAMPLE_RATE_8000) ||
            ((*(*audioRendererChangeInfos[0]->outputDeviceInfo.audioStreamInfo_.rbegin()).samplingRate.begin()
            <= SAMPLE_RATE_96000)));
        EXPECT_EQ((*audioRendererChangeInfos[0]->outputDeviceInfo.audioStreamInfo_.rbegin()).encoding,
            AudioEncodingType::ENCODING_PCM);
        std::set<AudioChannel> channels;
        (*audioRendererChangeInfos[0]->outputDeviceInfo.audioStreamInfo_.rbegin()).GetChannels(channels);
...
HWTEST_F(AudioStreamManagerUnitTest, AudioStreamChangeListnerGetCurrentCapturerChangeDeviceInfos_001, TestSize.Level1)
{
    ...
    EXPECT_EQ(true,
        (*(*audioCapturerChangeInfos[0]->inputDeviceInfo.audioStreamInfo_.rbegin()).samplingRate.rbegin()
        >= SAMPLE_RATE_8000)
        || ((*(*audioCapturerChangeInfos[0]->inputDeviceInfo.audioStreamInfo_.rbegin()).samplingRate.begin()
        <= SAMPLE_RATE_96000)));
    EXPECT_EQ((*audioCapturerChangeInfos[0]->inputDeviceInfo.audioStreamInfo_.rbegin()).encoding,
        AudioEncodingType::ENCODING_PCM);
    std::set<AudioChannel> channels;
    (*audioCapturerChangeInfos[0]->inputDeviceInfo.audioStreamInfo_.rbegin()).GetChannels(channels);
    EXPECT_EQ(true, (*channels.rbegin() >= MONO) && (*channels.begin() <= CHANNEL_8));
