...
uint32_t HdiAdapterManager::GetId(uint32_t base, uint32_t type, const std::string &info, bool isResident)
{
...
int32_t HdiAdapterManager::ProcessCaptureSource(const std::function<bool(uint32_t)> &limitFunc,
    const std::function<int32_t(std::shared_ptr<IAudioCaptureSource>)> &processFunc)
{
    int32_t ret = SUCCESS;
    auto func = [&ret, &limitFunc, &processFunc](const std::pair<const uint32_t, CaptureSourceInfo> &item) -> void {
        uint32_t captureId = item.first;
        if (!limitFunc(captureId)) {
            return;
        }
        if (processFunc(item.second.source_) != SUCCESS) {
...
std::shared_ptr<IDeviceManager> HdiAdapterManager::GetDeviceManager(uint32_t type)
{
...
void HdiAdapterManager::ReleaseDeviceManager(uint32_t type)
{
...
void HdiAdapterManager::RegistSinkCallback(uint32_t type, std::shared_ptr<IAudioSinkCallback> cb,
    const std::function<bool(uint32_t)> &limitFunc)
{
    CHECK_AND_RETURN_LOG(type < HDI_CB_TYPE_NUM, "invalid type");
...
void HdiAdapterManager::RegistSinkCallback(uint32_t type, IAudioSinkCallback *cb,
    const std::function<bool(uint32_t)> &limitFunc)
{
    CHECK_AND_RETURN_LOG(type < HDI_CB_TYPE_NUM, "invalid type");
    CHECK_AND_RETURN_LOG(cb != nullptr, "callback of type %{public}u is nullptr", type);
...
void HdiAdapterManager::RegistSourceCallback(uint32_t type, std::shared_ptr<IAudioSourceCallback> cb,
    const std::function<bool(uint32_t)> &limitFunc)
{
    CHECK_AND_RETURN_LOG(type < HDI_CB_TYPE_NUM, "invalid type");
    CHECK_AND_RETURN_LOG(cb != nullptr, "callback of type %{public}u is nullptr", type);
...
void HdiAdapterManager::RegistSourceCallback(uint32_t type, IAudioSourceCallback *cb,
    const std::function<bool(uint32_t)> &limitFunc)
{
    CHECK_AND_RETURN_LOG(type < HDI_CB_TYPE_NUM, "invalid type");
    CHECK_AND_RETURN_LOG(cb != nullptr, "callback of type %{public}u is nullptr", type);
...
int32_t HdiAdapterManager::LoadAdapter(uint32_t deviceManagerType, const std::string &adapterName)
{
    std::shared_ptr<IDeviceManager> deviceManager = GetDeviceManager(deviceManagerType);
    CHECK_AND_RETURN_RET(deviceManager != nullptr, ERR_INVALID_HANDLE);
    return deviceManager->LoadAdapter(adapterName);
}

void HdiAdapterManager::UnloadAdapter(uint32_t deviceManagerType, const std::string &adapterName, bool force)
{
    std::shared_ptr<IDeviceManager> deviceManager = GetDeviceManager(deviceManagerType);
