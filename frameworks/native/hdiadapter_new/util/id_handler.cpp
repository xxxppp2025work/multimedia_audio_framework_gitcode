...
uint32_t IdHandler::GetId(uint32_t base, uint32_t type, const std::string &info)
{
...
uint32_t IdHandler::GetRenderIdByDeviceClass(const std::string &deviceClass, const std::string &info)
{
    ...
    if (deviceClass == "primary") {
        return GetId(HDI_ID_BASE_RENDER, HDI_ID_TYPE_PRIMARY, HDI_ADAPTER_ID_INFO_DEFAULT);
    } else if (deviceClass == "usb") {
        return GetId(HDI_ID_BASE_RENDER, HDI_ID_TYPE_PRIMARY, HDI_ADAPTER_ID_INFO_USB);
    } else if (deviceClass == "dp") {
        return GetId(HDI_ID_BASE_RENDER, HDI_ID_TYPE_PRIMARY, HDI_ADAPTER_ID_INFO_DP);
    } else if (deviceClass == "a2dp") {
        return GetId(HDI_ID_BASE_RENDER, HDI_ID_TYPE_BLUETOOTH, HDI_ADAPTER_ID_INFO_DEFAULT);
    } else if (deviceClass == "a2dp_fast") {
        return GetId(HDI_ID_BASE_RENDER, HDI_ID_TYPE_BLUETOOTH, HDI_ADAPTER_ID_INFO_MMAP);
#ifdef FEATURE_FILE_IO
    } else if (deviceClass == "file_io") {
        return GetId(HDI_ID_BASE_RENDER, HDI_ID_TYPE_FILE, HDI_ADAPTER_ID_INFO_DEFAULT);
#endif
        ...
        return GetId(HDI_ID_BASE_RENDER, HDI_ID_TYPE_OFFLOAD, HDI_ADAPTER_ID_INFO_DEFAULT);
    } else if (deviceClass == "multichannel") {
        return GetId(HDI_ID_BASE_RENDER, HDI_ID_TYPE_MULTICHANNEL, HDI_ADAPTER_ID_INFO_DEFAULT);
    } else if (deviceClass == "primary_direct_voip") {
        return GetId(HDI_ID_BASE_RENDER, HDI_ID_TYPE_PRIMARY, HDI_ADAPTER_ID_INFO_VOIP);
    } else if (deviceClass == "primary_mmap_voip") {
        return GetId(HDI_ID_BASE_RENDER, HDI_ID_TYPE_FAST, HDI_ADAPTER_ID_INFO_VOIP);
    }
    ...
    const std::string &info)
{
    ...
    if (deviceClass == "primary") {
        ...
        return GetId(HDI_ID_BASE_CAPTURE, HDI_ID_TYPE_PRIMARY, HDI_ADAPTER_ID_INFO_DEFAULT);
    } else if (deviceClass == "usb") {
        return GetId(HDI_ID_BASE_CAPTURE, HDI_ID_TYPE_PRIMARY, HDI_ADAPTER_ID_INFO_USB);
    } else if (deviceClass == "a2dp") {
        return GetId(HDI_ID_BASE_CAPTURE, HDI_ID_TYPE_BLUETOOTH, HDI_ADAPTER_ID_INFO_DEFAULT);
#ifdef FEATURE_FILE_IO
    } else if (deviceClass == "file_io") {
        return GetId(HDI_ID_BASE_CAPTURE, HDI_ID_TYPE_FILE, HDI_ADAPTER_ID_INFO_DEFAULT);
#endif
#ifdef FEATURE_DISTRIBUTE_AUDIO
    } else if (deviceClass == "remote") {
...
bool IdHandler::CheckId(uint32_t id, uint32_t requireBase)
{
