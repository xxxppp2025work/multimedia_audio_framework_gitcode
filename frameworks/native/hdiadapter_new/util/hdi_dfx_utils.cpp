...
void HdiDfxUtils::DumpData(char *data, uint64_t &len, const IAudioSourceAttr &attr, DfxAttr &dfxAttr)
{
    ...
    VolumeTools::DfxOperation(buffer, streamInfo, dfxAttr.logUtilsTag_, dfxAttr.volumeDataCount_);
    CHECK_AND_RETURN(AudioDump::GetInstance().GetVersionType() == DumpFileUtil::BETA_VERSION);
    DumpFileUtil::WriteDumpFile(dfxAttr.dumpFile_, static_cast<void *>(data), len);
    AudioCacheMgr::GetInstance().CacheData(dfxAttr.dumpFileName_, static_cast<void *>(data), len);
}
} // namespace AudioStandard
} // namespace OHOS
