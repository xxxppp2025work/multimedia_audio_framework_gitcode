...
public:
    ...
    int32_t PreloadUsb(const std::string &usbInfo) override;

    int32_t UpdateActiveDevice(DeviceType inputDevice) override;
    int32_t UpdateSourceType(SourceType sourceType) override;
    void RegistCallback(IAudioSourceCallback *callback) override;
    void RegistStateCallback(std::unique_ptr<IAudioSourceCallback> callback) override;
...
private:
    ...
    int32_t UpdateUsbAttr(const std::string &usbInfo);
...
private:
    ...
    std::unique_ptr<IAudioSourceCallback> stateCallback_ = nullptr;
