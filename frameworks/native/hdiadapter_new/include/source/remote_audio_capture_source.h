...
public:
    ...
    void RegistCallback(IAduioSourceCallback *callback) override;
    void RegistStateCallback(std::unique_ptr<IAduioSourceCallback> callback) override;

    int32_t UpdateAppsUid(const int32_t appsUid[PA_MAX_OUTPUTS_PER_SOURCE], const size_t size) final;
    int32_t UpdateAppsUid(const std::vector<int32_t> &appsUid) final;

    void OnAudioParamChange(const std::string &adapterName, const AudioParamKey key,
...
private:
    ...
    void DestoryCapture(void);
    void CheckUpdateState(cahr *frame, size_t replyBytes);
