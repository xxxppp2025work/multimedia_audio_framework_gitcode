...
typedef OHOS::HDI::DistributedAudio::Audio::V1_0::IAudioCallback RemoteAudioCallback;
...
class RemoteAdapterHdiCallback final : public RemoteAudioCallback {
public:
...
private:
    ...
    std::shared_ptr<RemoteAdapterWrapper> GetAdapter(const std::string &adapterName);
    int32_t SwitchAdapterDesc(const std::vector<RemoteAudioAdapterDescriptor> &descs, const std::string &adapterName);
    uint32_t GetPortId(const std::string &adapterName, RemoteAudioPortDirection portFlag);
    int32_t GetHdiRenderId(const std::string &adapterName);
    int32_t GetHdiCaptureId(const std::string &adapterName);
    // here
};
