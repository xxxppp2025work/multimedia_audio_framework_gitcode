...
#include CHECK_ADAPTER(adapterName) \
    CHECK_AND_RETURN_LOG(adapters_.count((adapterName)) != 0 && adapters_[(adapterName)] != nullptr && \
    adapters_[(adapterName)]->adapter_ != nullptr, "adapter %{public}s is nullptr", (adapterName)); \

#include CHECK_ADAPTER_RET(adapterName, err) \
    CHECK_AND_RETURN_RET_LOG(adapters_.count((adapterName)) != 0 && adapters_[(adapterName)] != nullptr && \
    adapters_[(adapterName)]->adapter_ != nullptr, (err), "adapter %{public}s is nullptr", (adapterName)); \
