...
public:
    ...
    void RegistSinkCallback(uint32_t type, std::shared_ptr<IAudioSinkCallback> cb,
        const std::function<bool(uint32_t)> &limitFunc = [](uint32_t id) -> bool { return false; });
    void RegistSinkCallback(uint32_t type, IAudioSinkCallback *cb,
        const std::function<bool(uint32_t)> &limitFunc = [](uint32_t id) -> bool { return false; });
    void RegistSourceCallback(uint32_t type, std::shared_ptr<IAudioSourceCallback> cb,
        const std::function<bool(uint32_t)> &limitFunc = [](uint32_t id) -> bool { return false; });
    void RegistSourceCallback(uint32_t type, IAudioSourceCallback *cb,
        const std::function<bool(uint32_t)> &limitFunc = [](uint32_t id) -> bool { return false; });
