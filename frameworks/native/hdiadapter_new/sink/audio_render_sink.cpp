...
AudioRenderSink::AudioRenderSink(const std::string &halName)
    : halName_(halName)
{
    if (halName_ == HDI_ADAPTER_ID_INFO_DIRECT || halName_ == HDI_ADAPTER_ID_INFO_VOIP) {
...
std::string AudioRenderSink::GetAudioParameter(const AudioParamKey key, const std::string &condition)
{
    ...
    AUDIO_INFO_LOG("key: %{public}d, condition: %{public}s, halName: %{public}s", key, condition.c_str(),
        halName_.c_str());
    if (condition.starts_with("get_usb_info#C") && halName_ == HDI_ADAPTER_ID_INFO_USB) {
    ...
    if (key == AudioParamKey::GET_DP_DEVICE_INFO && halName_ == HDI_ADAPTER_ID_INFO_DP) {
...
int32_t AudioRenderSink::SetSinkMuteForSwitchDevice(bool mute)
{
    ...
    if (mute) {
        ...
        if (halName_ == HDI_ADAPTER_ID_INFO_VOIP) {
            SetVolume(0.0f, 0.0f);
        }
    } else {
        muteCount_--;
...
int32_t AudioRenderSink::PreloadUsb(const std::string &usbInfo)
{
    CHECK_AND_RETURN_RET_LOG(halName_ == HDI_ADAPTER_ID_INFO_USB, ERR_INVALID_OPERATION, "only supported for usb");
    int32_t ret = UpdateUsbAttr(usbInfo);
    CHECK_AND_RETURN_RET_LOG(ret == SUCCESS, ret, "init attr fail");
    ret = InitRender();
    CHECK_AND_RETURN_RET_LOG(ret == SUCCESS, ret, "init render fail");
    return SUCCESS;
}
...
void AudioRenderSink::RegistCallback(IAudioSinkCallback *callback)
{
    std::lock_guard<std::mutex> lock(sinkMutex_);
    if (callback_) {
        AUDIO_WARNING_LOG("already registed");
        return;
    }
    callback_ = callback;
...
void AudioRenderSink::InitAudioSampleAttr(struct AudioSampleAttributes &param)
{
    ...
    if (halName_ == HDI_ADAPTER_ID_INFO_DP) {
        param.type = AUDIO_DP;
    } else if (halName_ == HDI_ADAPTER_ID_INFO_DIRECT) {
...
void AudioRenderSink::InitSceneDesc(struct AudioSceneDescriptor &sceneDesc, AudioScene audioScene)
{
    ...
    if (halName_ == HDI_ADAPTER_ID_INFO_DIRECT) {
...
int32_t AudioRenderSink::UpdateUsbAttr(const std::string &usbInfo)
{
    CHECK_AND_RETURN_RET_LOG(!usbInfo.empty(), ERR_INVALID_PARAM, "usb info is empty");

    auto sinkRate_begin = usbInfo.find("sink_rate:");
    auto sinkRate_end = usbInfo.find_first_of(";", sinkRate_begin);
    std::string sampleRateStr = usbInfo.substr(sinkRate_begin + std::strlen("sink_rate:"),
        sinkRate_end - sinkRate_begin - std::strlen("sink_rate:"));
    auto sinkFormat_begin = usbInfo.find("sink_format:");
    auto sinkFormat_end = usbInfo.find_first_of(";", sinkFormat_begin);
    std::string formatStr = usbInfo.substr(sinkFormat_begin + std::strlen("sink_format:"),
