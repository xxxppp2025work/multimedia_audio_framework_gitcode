...
int32_t MultichannelAudioRenderSink::Start(void)
{
    ...
    AUDIO_INFO_LOG("start success, frameSize: %{public}lu, frameCount: %{public}lu", frameSize, frameCount);
...
int32_t MultichannelAudioRenderSink::PreloadUsb(const std::string &usbInfo)
{
...
void MultichannelAudioRenderSink::RegistCallback(IAudioSinkCallback *callback)
{
    std::lock_guard<std::mutex> lock(sinkMutex_);
    if (callback_) {
...
int32_t MultichannelAudioRenderSink::UpdateUsbAttr(const std::string &usbInfo)
{
