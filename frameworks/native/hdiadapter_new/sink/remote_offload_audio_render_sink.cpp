...
int32_t RemoteOffloadHdiCallbackImpl::RenderCallback(AudioCallbackType type, int8_t &reserved, int8_t &cookie)
{
    ...
    if (!sink_->started_.load() || sink_->isFlushing_) {
...
int32_t RemoteOffloadAudioRenderSink::SetOffloadRenderCallbackType(RenderCallbackType type)
{
    ...
    CHECK_AND_RETURN_RET_LOG(started_.load() && !isFlushing_, ERR_ILLEGAL_STATE,
...
uint32_t RemoteOffloadAudioRenderSink::PcmFormatToBit(AudioSampleFormat format)
{
    ...
    switch (hdiFormat) {
        ...
        case AUDIO_FORMAT_TYPE_PCM_32_BIT:
            return PCM_32_BIT;
        case AUDIO_FORMAT_TYPE_PCM_FLOAT:
            return PCM_32_BIT;
        default:
...
void RemoteOffloadAudioRenderSink::InitAudioSampleAttr(struct AudioSampleAttributes &param)
{
...
void RemoteOffloadAudioRenderSink::InitDeviceDesc(struct AudioDeviceDescriptor &deviceDesc)
{
