...
int32_t VACaptureSource::CreateCapture()
{
	...
	std::string macAddress = attr_.deviceNetworkId;
	...
	CHECK_AND_RETURN_RET_LOG(deviceController_ != nullptr, ERR_OPERATION_FAILED, "convert to IVADeviceController failed");
...
{
	CHECK_AND_RETURN_RET_LOG(inputStream_ != nullptr, ERR_OPERATION_FAILED, "inputStream is nullptr");
	...
	CHECK_AND_RETURN_RET_LOG(bufferOperator_ != nullptr, ERR_OPERATION_FAILED, "bufferOperator_ is null");
...
void VACaptureSource::DeInit(void)
{
	...
	CHECK_AND_RETURN_LOG(inputStream_ != nullptr, "inputStream_ is null");
	inputStream_->Close();
...
int32_t VACaptureSource::Start(void)
{
	...
	CHECK_AND_RETURN_RET_LOG(inputStream_ != nullptr, ERR_OPERATION_FAILED, "inputStream_ is null");
...
void VACaptureSource::PrintDfx(int64_t useTime)
{
	auto ret = HiSysEventWrite(HiviewDFX::HiSysEvent::Domain::AUDIO,
							   ...
							   false,
							   "STREAM_TYPE",
							   AudioStreamType::STREAM_RECORDING,
							   "DURATION",
							   (int32_t)useTime);
	if (ret) {
