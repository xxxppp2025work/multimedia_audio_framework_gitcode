...
{
	CHECK_AND_RETURN_RET_LOG(bufferOperator_ != nullptr && inputStream_ != nullptr, ERR_OPERATION_FAILED, "wrong status");
	std::lock_guard<std::mutex> lock(statusMutex_);
...
std::string VACaptureSource::GetAudioParameter(const AudioParamKey key, const std::string &condition)
{
	...
	AUDIO_INFO_LOG("get audio parameter from va device: {\"key\":\"%{public}d\", \"value\":\"%{public}s\"}", key, value.c_str());
	return value;
}

void VACaptureSource::SetAudioParameter(const AudioParamKey key, const std::string &condition, const std::string &value)
{
	AUDIO_INFO_LOG("set audio parameter for va device: {\"key\":\"%{public}d\", \"value\":\"%{public}s\"}", key, value.c_str());
