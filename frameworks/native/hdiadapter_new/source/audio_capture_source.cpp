...
int32_t AudioCaptureSource::Start(void)
{
    ...
    if (stateCallback_ != nullptr) {
        stateCallback_->OnCaptureState(true);
    }
...
int32_t AudioCaptureSource::PreloadUsb(const std::string &usbInfo)
{
    ...
    ret = InitCapture();
    CHECK_AND_RETURN_RET_LOG(ret == SUCCESS, ret, "init capture fail");
    return SUCCESS;
}
...
void AudioCaptureSource::RegistCallback(IAudioSourceCallback *callback)
{
    AUDIO_INFO_LOG("in");
    callback_ = callback;
}

void AudioCaptureSource::RegistStateCallback(std::unique_ptr<IAudioSourceCallback> callback)
{
    AUDIO_INFO_LOG("in");
...
{
    dumpString += "type: PrimarySource\tstarted: " + std::string(started_ ? "true" : "false") + "\thalName: " + halName_ +
        "\tcurrentActiveDevice: " + std::to_string(currentActiveDevice_) + "\tsourceType: " +
...
int32_t AudioCaptureSource::UpdateUsbAttr(const std::string &usbInfo)
{
    CHECK_AND_RETURN_RET_LOG(!usbInfo.empty(), ERR_INVALID_PARAM, "usb info is empty");

    auto sourceRate_begin = usbInfo.find("source_rate:");
    auto sourceRate_end = usbInfo.find_first_of(";", sourceRate_begin);
    std::string sampleRateStr = usbInfo.substr(sourceRate_begin + std::strlen("source_rate:"),
        sourceRate_end - sourceRate_begin - std::strlen("source_rate:"));
    auto sourceFormat_begin = usbInfo.find("source_format:");
    auto sourceFormat_end = usbInfo.find_first_of(";", sourceFormat_begin);
    std::string formatStr = usbInfo.substr(sourceFormat_begin + std::strlen("source_format:"),
...
int32_t AudioCaptureSource::DoStop(void)
{
    ...
    started_ = false;
    if (stateCallback_ != nullptr) {
        stateCallback_->OnCaptureState(false);
    }
    return SUCCESS;
}
