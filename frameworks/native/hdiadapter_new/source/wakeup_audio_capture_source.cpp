...
void WakeupBuffer::Offer(const char *frame, const uint64_t bufferBytes)
{
    ...
    if (tail < head_) {
        ...
        MemcpysAndCheck((buffer_.get() + tail), MAX_BUFFER_SIZE - tail, frame, copySize);
...
void WakeupAudioCaptureSource::RegistCallback(IAudioSourceCallback *callback)
{
    return audioCaptureSource_.RegistCallback(callback);
}

void WakeupAudioCaptureSource::RegistStateCallback(std::unique_ptr<IAudioSourceCallback> callback)
{
    return audioCaptureSource_.RegistStateCallback(std::move(callback));
}
