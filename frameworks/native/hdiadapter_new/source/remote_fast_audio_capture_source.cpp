...
int32_t RemoteFastAudioCaptureSource::SetAudioScene(AudioScene audioScene, DeviceType activeDevice)
{
    (void)activeDevice;
...
void RemoteFastAudioCaptureSource::RegistCallback(IAudioSourceCallback *callback)
{
    AUDIO_INFO_LOG("in");
    callback_ = callback;
}

void RemoteFastAudioCaptureSource::RegistStateCallback(std::unique_ptr<IAudioSourceCallback> callback)
{
    AUDIO_INFO_LOG("not support");
}
