...
std::shared_ptr<IDeviceAdapter> RemoteDeviceManager::LoadAdapter(const std::string &adapterName, bool needReInitManager)
{
    ...
    CHECK_AND_RETURN_RET_LOG(ret == SUCCESS && descs.data() != nullptr, nullptr, "get adapters fail");
    ...
    std::shared_ptr<IDeviceAdapter> deviceAdapter = std::make_shared<RemoteDeviceAdapter>(adapterName, adapter,
        descs[index]);
    AUDIO_INFO_LOG("load adapter %{public}s success", adapterName.c_str());
    return deviceAdapter;
}

void RemoteDeviceManager::UnloadAdapter(std::shared_ptr<IDeviceAdapter> deviceAdapter)
{
    CHECK_AND_RETURN_LOG(deviceAdapter != nullptr &&
        deviceAdapter->GetDeviceManagerType() == HDI_DEVICE_MANAGER_TYPE_REMOTE, "invalid adapter");
    CHECK_AND_RETURN_LOG(audioManager_ != nullptr, "audio manager is nullptr");

    auto remoteDeviceAdapter = std::dynamic_pointer_cast<RemoteDeviceAdapter>(deviceAdapter);
    CHECK_AND_RETURN_LOG(remoteDeviceAdapter != nullptr &&
        remoteDeviceAdapter->adapter_ != nullptr, "adapter is nullptr");
    if (remoteDeviceAdapter->routeHandle_ != -1) {
        remoteDeviceAdapter->adapter_->ReleaseAudioRoute(remoteDeviceAdapter->routeHandle_);
    }
    audioManager_->UnloadAdapter(remoteDeviceAdapter->adapterDesc_.adapterName);
