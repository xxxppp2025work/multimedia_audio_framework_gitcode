...
public:
    ...
    void TestAction(uint32_t type);
    void TestSetAndGet(uint32_t type);

protected:
    std::string adapterName_ = "test";
};

void AdapterUnitTest::TestAction(uint32_t type)
{
...
void AdapterUnitTest::TestSetAndGet(uint32_t type)
{
    std::shared_ptr<IDeviceManager> deviceManager = HdiAdapterManager::GetInstance().GetDeviceManager(type);
    ASSERT_NE(deviceManager, nullptr);
