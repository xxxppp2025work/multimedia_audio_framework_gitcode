...
AniAudioManager::~AniAudioManager()
{
    ANI_ERR_LOG("AniAudioManager::~AniAudioManager()");
}
...
ani_object AniAudioManager::Constructor([[maybe_unused]] ani_env *env, [[maybe_unused]] ani_class clazz)
{
    ...
    const std::string &cbName, ani_object info, ani_object callback)
{
    ...
    ANI_CHECK_RETURN_RET_LOG(audioManagerAni != nullptr,
        AniAudioError::ThrowError(env, ANI_ERR_NO_MEMORY), "audioManagerAni is nullptr");
    ANI_CHECK_RETURN_RET_LOG(audioManagerAni->audioMngr_ != nullptr,
    ...
    if (!cbName.compare(INTERRUPT_CALLBACK_NAME)) {
        ...
        ANI_INFO_LOG("enter RegisterVolumeChangeCallback");
...
void AniAudioManager::On(ani_env *env, ani_object object, ani_string type, ani_object info, ani_object callback)
{
...
void AniAudioManager::RegisterVolumeChangeCallback(ani_env *env, ani_object callback, AniAudioManager *audioManagerAni)
{
    if (audioManagerAni->volumeKeyEventCallbackAni_ == nullptr) {
        ...
        ANI_CHECK_RETURN_RET_LOG(ret == SUCCESS, AniAudioError::ThrowError(env, ret),
