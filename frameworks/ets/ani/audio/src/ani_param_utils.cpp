...
ani_status AniParamUtils::GetInt32(ani_env *env, ani_object arg, int32_t &value)
{
    ...
    ani_class cls {};
    static const std::string className = "C{std.core.Int}";
    ani_status status = env->FindClass(className.c_str(), &cls);
    if (status != ANI_OK) {
        AUDIO_ERR_LOG("Can't find C{std.core.Int}.");
        return status;
    }
    ...
    if (status != ANI_OK) {
        AUDIO_ERR_LOG("Can't find method unboxed in C{std.core.Int}.");
        return status;
    }
...
{
    ani_class clas {};
    static const std::string className = "C{escompat.Array}";
    ...
    if (status != ANI_OK) {
        AUDIO_ERR_LOG("Can't find method <ctor> in C{escompat.Array}.");
        return status;
    }
    ...
    if (status != ANI_OK) {
        AUDIO_ERR_LOG("Can't find method set in C{escompat.Array}.");
        return status;
    }
...
{
    static const char *className = "C{std.core.Double}";
    ani_class cls {};
    ani_status status = env->FindClass(className, &cls);
    if (status != ANI_OK) {
...
{
    static const char *className = "C{std.core.Boolean}";
    ani_class cls {};
    ani_status status = env->FindClass(className, &cls);
    if (status != ANI_OK) {
        AUDIO_ERR_LOG("Failed to find class: %{public}s", className);
...
{
    static const char *className = "C{escompat.BigInt}";
    ani_class cls {};
    ani_status status = env->FindClass(className, &cls);
    if (status != ANI_OK) {
        AUDIO_ERR_LOG("Failed to find class: %{public}s", className);
    ...
    const std::string &errMsg)
{
    ...
    static const std::string className = "C{@ohos.multimedia.audio.audio.AudioAniError}";
    ani_class cls;
    ani_status status = env->FindClass(className.c_str(), &cls);
    if (status != ANI_OK) {
