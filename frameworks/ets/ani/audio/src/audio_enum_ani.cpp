...
ani_status AudioEnumAni::ToAniEnum(ani_env *env, AudioEncodingType value, ani_enum_item &aniEnumItem)
{
    ...
    CHECK_COND_RET(it != ANI_AUDIO_ENCODING_TYPE_INDEX_MAP.end(), ANI_INVALID_ARGS,
        "Unsupport enum: %{public}d", value);
    ani_int enumIndex = static_cast<ani_int>(it->second);

    ani_enum aniEnum {};
    CHECK_STATUS_RET(env->FindEnum(ANI_CLASS_AUDIO_ENCODING_TYPE.c_str(), &aniEnum), "Find Enum Fail");
    CHECK_STATUS_RET(env->Enum_GetEnumItemByIndex(aniEnum, enumIndex, &aniEnumItem), "Find Enum item Fail");
    return ANI_OK;
}
...
ani_status AudioEnumAni::ToAniEnum(ani_env *env, DeviceBlockStatus value, ani_enum_item &aniEnumItem)
{
    ...
    CHECK_COND_RET(it != ANI_DEVICE_BLOCK_STATUS_INDEX_MAP.end(), ANI_INVALID_ARGS,
        "Unsupport enum: %{public}d", value);
    ani_int enumIndex = static_cast<ani_int>(it->second);

    ani_enum aniEnum {};
    CHECK_STATUS_RET(env->FindEnum(ANI_CLASS_DEVICE_BLOCK_STATUS.c_str(), &aniEnum), "Find Enum Fail");
    CHECK_STATUS_RET(env->Enum_GetEnumItemByIndex(aniEnum, enumIndex, &aniEnumItem), "Find Enum item Fail");
    return ANI_OK;
}
