...
struct DeviceStreamInfo {
    ...
    static std::string SerializeList(const std::list<DeviceStreamInfo> &streamInfoList)
    {
        ...
        for (const auto &streamInfo : streamInfoList) {
            uint32_t len = streamInfoList.size();
            res.append(reinterpret_cast<char *>(&len), sizeof(len));
            res += streamInfo.Serialize();
        }
        return res;
    }
    ...
    {
        std::list<DeviceStreamInfo> streamInfoList;
        uint32_t idx = 0;

        while (idx < data.size()) {
            uint32_t len = 0;
            auto ret = memcpy_s(&len, sizeof(len), &data[idx], sizeof(len));
            if (ret != SUCCESS) {
                return streamInfoList;
            }
            idx += sizeof(len);

            std::string subStr = data.substr(idx, len);
            idx += len;

            DeviceStreamInfo streamInfo;
            bool desRet = streamInfo.Deserialize(subStr);
            if (!desRet) {
                return streamInfoList;
            }
    ...
    bool Deserialize(const std::string& data)
    {
        ...
        return DeserializeEnum(iss, encoding) &&
    ...
    template <typename T>
    void SerializeEnum(std::ostream &os, T value) const
    {
    ...
    template <typename T>
    bool DeserializeEnum(std::istream &is, T &value)
    {
        int32_t intValue;
    ...
    template <typename T>
    void SerializeSet(std::ostream &os, const std::set<T> &s) const
    {
    ...
    template <typename T>
    bool DeserializeSet(std::istream &is, std::set<T> &s)
    {
        uint32_t len = 0;
