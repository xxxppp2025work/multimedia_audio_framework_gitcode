...
public:
    ...
    int32_t SetStreamMute(std::shared_ptr<AudioDeviceDescriptor> &device, AudioStreamType streamType, bool mute, StreamUsage streamUsage = STREAM_USAGE_UNKNOWN,
        const DeviceType &deviceType = DEVICE_TYPE_NONE);
...
private:
    ...
    int32_t SetStreamMuteInternal(AudioStreamType streamType, bool mute, StreamUsage streamUsage,
        const DeviceType &deviceType = DEVICE_TYPE_NONE);
    int32_t SetStreamMuteInternal(std::shared_ptr<AudioDeviceDescriptor> &device, AudioStreamType streamType, bool mute, StreamUsage streamUsage,
        const DeviceType &deviceType = DEVICE_TYPE_NONE);
    ...
    bool IsCurDeviceNeedSaveVolumeToDatabase(std::shared_ptr<AudioDeviceDescriptor> &device);
