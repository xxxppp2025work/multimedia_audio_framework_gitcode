...
void AudioPolicyService::UpdateTrackerDeviceChange(const vector<sptr<AudioDeviceDescriptor>> &desc)
{
    ...
    for (sptr<AudioDeviceDescriptor> deviceDesc : desc) {
        if (deviceDesc->deviceRole_ == OUTPUT_DEVICE) {
            ...
            if (itr != connectedDevices_.end()) {
                AudioDeviceDescriptor outputDevice = AudioDeviceDescriptor(AudioDeviceDescriptor::DEVICE_INFO);
        ...
        if (deviceDesc->deviceRole_ == INPUT_DEVICE) {
            ...
            if (itr != connectedDevices_.end()) {
                AudioDeviceDescriptor inputDevice = AudioDeviceDescriptor(AudioDeviceDescriptor::DEVICE_INFO);
...
int32_t AudioPolicyService::GetVoipRecordDeviceInfo(const AudioProcessConfig &config, AudioDeviceDescriptor &deviceInfo)
{
