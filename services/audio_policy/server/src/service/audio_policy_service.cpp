    ...
    sptr<AudioDeviceDescriptor> localDeviceDescriptor)
{
    ...
    for (size_t i = 0; i < sinkInputIds.size(); i++) {
        ...
        AUDIO_WARNING_LOG("move for session [%{public}d], portName %{public}s pipeType %{public}d",
...
bool AudioPolicyService::NotifyRecreateRendererStream(std::unique_ptr<AudioDeviceDescriptor> &desc,
    const std::shared_ptr<AudioRendererChangeInfo> &rendererChangeInfo, const AudioStreamDeviceChangeReasonExt reason)
{
    AUDIO_WARNING_LOG("New device type: %{public}d, current rendererFlag: %{public}d, origianl flag: %{public}d",
...
bool AudioPolicyService::NotifyRecreateCapturerStream(bool isUpdateActiveDevice,
    ...
    const AudioStreamDeviceChangeReasonExt reason)
{
    AUDIO_WARNING_LOG("Is update active device: %{public}d, current capturerFlag: %{public}d, origianl flag: %{public}d",
...
#ifdef BLUETOOTH_ENABLE
    AUDIO_WARNING_LOG("a2dp device name [%{public}s]", (deviceDescriptor->deviceName_).c_str());
...
void AudioPolicyService::OnDeviceInfoUpdated(AudioDeviceDescriptor &desc, const DeviceInfoUpdateCommand command)
{
    ...
    AUDIO_WARNING_LOG("[%{public}s] type[%{public}d] command: %{public}d category[%{public}d] connectState[%{public}d] " \
    ...
    if (command == ENABLE_UPDATE && desc.isEnable_ == true) {
        ...
        if ((userSelectMediaDevice->deviceType_ == desc.deviceType_ &&
            ...
            AUDIO_WARNING_LOG("Current enable state has been set true during user selection, no need to be set again.");
            return;
        }
