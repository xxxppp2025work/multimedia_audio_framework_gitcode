...
void AudioPolicyService::OnMicrophoneBlockedUpdate(DeviceType devType, OH_AudioDevice_BlockStatus status)
{
    CHECK_AND_RETURN_LOG(devType != DEVICE_TYPE_NONE, "devType is none type");
    if (g_adProxy == nullptr) {
        GetAudioServerProxy();
...
void AudioPolicyService::OnBlockedStatusUpdated(DeviceType devType, OH_AudioDevice_BlockStatus status)
{
    std::vector<sptr<AudioDeviceDescriptor>> descForCb = {};
