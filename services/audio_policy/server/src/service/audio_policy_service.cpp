    ...
    StreamUsage streamUsage, std::shared_ptr<AudioDeviceDescriptor> desc)
{
    ...
    if (streamUsage == STREAM_USAGE_VOICE_COMMUNICATION ||
...
{
    std::shared_ptr<AudioDeviceDescriptor> intermediateDescriptor =
        std::make_shared<AudioDeviceDescriptor>(descriptor);
    ...
    AudioCapturerInfo &captureInfo, std::string networkId)
{
    ...
    if (captureInfo.sourceType <= SOURCE_TYPE_INVALID ||
        ...
        std::shared_ptr<AudioDeviceDescriptor> devDesc =
            std::make_shared<AudioDeviceDescriptor>(GetCurrentInputDevice());
        deviceList.push_back(devDesc);
        return deviceList;
    }
...
void AudioPolicyService::UpdateDeviceInfo(
    AudioDeviceDescriptor &deviceInfo, const std::shared_ptr<AudioDeviceDescriptor> &desc,
...
void AudioPolicyService::WriteDeviceChangedSysEvents(
