...
static const int64_t OLD_DEVICE_UNAVALIABLE_MUTE_SLEEP_MS = 300000; // 300ms
static const int64_t DOUBLE_RINGING_MUTE_SLEEP_MS = 300000; // 300ms
    ...
    vector<std::unique_ptr<AudioDeviceDescriptor>>& outputDevices, const AudioStreamDeviceChangeReasonExt reason)
{
    ...
    if (outputDevices.size() != 1) {
        if (IsStreamActive(STREAM_MUSIC)) {
            ...
            MuteSinkPort("null", PRIMARY_SPEAKER, AudioStreamDeviceChangeReasonExt::ExtEnum::OVERRODE);
            AUDIO_INFO_LOG("double ringing mute primary wait %{public}d us", DOUBLE_RINGING_MUTE_SLEEP_MS);
            usleep(DOUBLE_RINGING_MUTE_SLEEP_MS);
        }
        return;
    }
