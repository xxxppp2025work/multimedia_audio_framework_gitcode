    ...
    const int32_t uid, const std::string caller)
{
    ...
    AUDIO_INFO_LOG("deviceType: %{public}d, uid: %{public}d, pid: %{public}d, name: %{public}s, caller: %{public}s",
        deviceDescriptor->deviceType_, callerUid, callerPid, bundleName.c_str(), caller.c_str());
    if (audioClientInfoMgrCallback_ != nullptr) {
        audioClientInfoMgrCallback_->OnCheckClientInfo(bundleName, callerUid, callerUid);
    }
