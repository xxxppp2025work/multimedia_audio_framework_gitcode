...
void AudioBackgroundManager::SubscribeBackgroundTask()
{
    ...
    int ret = BackgroundTaskMgr::BackgroundTaskMgrHelper::SubscribeBackgroundTask(*backgroundTaskListener_);
    if (ret != 0) {
        AUDIO_INFO_LOG("SubscribeBackgroundTask failed, err:%{public}d", ret);
    }
}
