...
int32_t AudioOffloadStream::MoveToNewPipeInner(uint32_t sessionId, AudioPipeType pipeType)
{
    ...
    switch (pipeType) {
        case PIPE_TYPE_OFFLOAD: {
            if (!CheckStreamOffloadMode(sessionId, streamType)) { return ERROR; }
            if (LoadOffloadModule() != SUCCESS) { return ERROR; }
        ...
        case PIPE_TYPE_MULTICHANNEL: {
            if (!CheckStreamMultichannelMode(sessionId)) { return ERROR; }
