...
int32_t AudioOffloadStream::MoveToNewPipeInner(uint32_t sessionId, AudioPipeType pipeType)
{
    ...
    int32_t ret = MoveToNewPipeInnerExt(sessionId, pipeType);

    return ret;
}

int32_t AudioOffloadStream::MoveToNewPipeInnerExt(const uint32_t sessionId, const AudioPipeType pipeType)
{
    int32_t ret = ERROR;
