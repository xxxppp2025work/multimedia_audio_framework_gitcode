...
static void LoadStreamUsageMapping(OriginalEffectConfig &result, xmlNode *thirdNode)
{
    ...
    while (currNode != nullptr) {
        int32_t audioEffectCountStreamUsageUpperLimit = 200;
        CHECK_AND_RETURN_LOG(countUsage < audioEffectCountStreamUsageUpperLimit,
            "streamUsage map item exceeds limit: %{public}d", audioEffectCountStreamUsageUpperLimit);
        if (currNode->type != XML_ELEMENT_NODE) {
            currNode = currNode->next;
            continue;
        }
