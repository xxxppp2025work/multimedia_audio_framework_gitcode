...
void AudioAdapterManager::InitVolumeMap(bool isFirstBoot)
{
    ...
    for (auto &deviceType: VOLUME_GROUP_TYPE_LIST) {
        for (auto &streamType: VOLUME_TYPE_LIST) {
            ...
            if (!volumeDataMaintainer_.GetVolume(deviceType, streamType)) {
                int32_t volumeLevel = GetDefaultVolumeLevel(streamType, deviceType);
...
int32_t AudioAdapterManager::GetDefaultVolumeLevel(AudioVolumeType volumeType, DeviceType deviceType) {
    AudioVolumeType volumeType = VolumeUtils::GetVolumeTypeFromStreamType(volumeType);
    int32_t defaultVolumeLevel = volumeLevelMapTemp[volumeType];
    int32_t defaultDeviceVolumeLevel = -1;
    auto it = DEVICE_TYPE_TO_DEVICE_VOLUME_TYPE_MAP.find(deviceType);
    if (it != DEVICE_TYPE_TO_DEVICE_VOLUME_TYPE_MAP.end()) {
        defaultDeviceVolumeLevel = streamVolumeInfos_[volumeType]->deviceVolumeInfos[it->second]->defaultLevel;
    }
