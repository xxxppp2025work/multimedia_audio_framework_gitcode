...
static const size_t DP_PORT_COUNT = 3;
static const std::string DP_PATH = "/sys/class/switch/hdmi_audio";
static const std::string DEVICE_PORT = "device_port=";
...
int32_t AudioSocketThread::DetectDPState(AudioEvent *audioEvent)
{
    for (size_t i = 0; i <= DP_PORT_COUNT; i++) {
        ...
        std::string namePatch = DP_PATH;

        if (i == 0) {
            statePath.append("/state");
            namePatch.append("/name");
        } else {
            statePath.append(std::to_string(i) + "/state");
            namePatch.append(std::to_string(i) + "/name");
        }
        ...
        ret = ReadAndScanDpName(namePatch, audioEvent->name);
...
int32_t AudioSocketThread::ReadAndScanDpState(const std::string &path, uint32_t &eventType)
{
    ...
    ret = fclose(fp);
    if (ret != 0) { AUDIO_ERR_LOG(" something wrong when fclose! err:%{public}d ", errno); }
    ...
    AUDIO_DEBUG_LOG("audio read dp state path:%{public}s event type:%{public}d", path.c_str(), eventType);
...
int32_t AudioSocketThread::ReadAndScanDpName(const std::string &path, std::string &name)
{
    ...
    AUDIO_DEBUG_LOG("audio read dp name path:%{public}s name:%{public}s", path.c_str(), deviceName);

    name = deviceName;
    auto portBegin = name.find(DEVICE_PORT);
    if (portBegin != name.npos) {
        name = name.substr(portBegin + std::strlen(DEVICE_PORT.c_str()));
