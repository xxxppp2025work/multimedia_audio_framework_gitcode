...
void AudioZoneService::RemoveDeviceFromGlobal(sptr<AudioDeviceDescriptor> device)
{
    ...
    AudioDeviceLock::GetInstance().OnDeviceStatusUpdated(*(device.GetRefPtr()), false);
}
    ...
    std::vector<sptr<AudioDeviceDescriptor>> devices)
{
    ...
    for (auto it : toGlobalDevices) {
        AudioDeviceLock::GetInstance().OnDeviceStatusUpdated(*(it.GetRefPtr()), true);
    }
    return SUCCESS;
}
...
std::vector<sptr<AudioDeviceDescriptor>> AudioZoneService::FetchOutputDevices(int32_t zoneId,
