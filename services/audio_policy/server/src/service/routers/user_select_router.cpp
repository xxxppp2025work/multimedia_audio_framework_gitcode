    ...
    int32_t clientUID)
{
    ...
    if (streamUsage == STREAM_USAGE_VOICE_RINGTONE || streamUsage == STREAM_USAGE_RINGTONE) {
        selectedDesc = GetCallRenderDevice(streamUsage, clientUID);
    } else {
        selectedDesc = GetMediaRenderDevice(streamUsage, clientUID);
    }

    if (!selectedDesc.get()) {
        AUDIO_INFO_LOG("Have no desc, just only add default device.");
        descs.push_back(make_shared<AudioDeviceDescriptor>());
        return descs;
    }
    if (selectedDesc->getType() == DEVICE_TYPE_NONE) {
        AUDIO_INFO_LOG("desc type is none, just only add default device.");
        descs.push_back(make_shared<AudioDeviceDescriptor>());
        return descs;
    }
    if (selectedDesc->GetDeviceCategory() == BT_SOUNDBOX) {
        AUDIO_INFO_LOG("desc Category is SOUNDBOX, just only add default device.");
        descs.push_back(make_shared<AudioDeviceDescriptor>());
        return descs;
    }
