...
{
    switch (code) {
        case static_cast<uint32_t>(AudioPolicyManagerInterfaceCode::REGISTER_AUDIO_ZONE_CLIENT):
            HandleRegisterAudioZoneClient(data, reply);
            break;
        case static_cast<uint32_t>(AudioPolicyManagerInterfaceCode::CREATE_AUDIO_ZONE):
            HandleCreateAudioZone(data, reply);
            break;
        case static_cast<uint32_t>(AudioPolicyManagerInterfaceCode::RELEASE_AUDIO_ZONE):
            HandleReleaseAudioZone(data, reply);
            break;
        case static_cast<uint32_t>(AudioPolicyManagerInterfaceCode::GET_ALL_AUDIO_ZONE):
            HandleGetAllAudioZone(data, reply);
            break;
        case static_cast<uint32_t>(AudioPolicyManagerInterfaceCode::GET_AUDIO_ZONE_BY_ID):
            HandleGetAudioZone(data, reply);
            break;
        case static_cast<uint32_t>(AudioPolicyManagerInterfaceCode::BIND_AUDIO_ZONE_DEVICE):
            HandleBindAudioZoneDevice(data, reply);
            break;
        case static_cast<uint32_t>(AudioPolicyManagerInterfaceCode::UNBIND_AUDIO_ZONE_DEVICE):
            HandleUnBindAudioZoneDevice(data, reply);
            break;
        case static_cast<uint32_t>(AudioPolicyManagerInterfaceCode::ENABLE_AUDIO_ZONE_REPORT):
            HandleEnableAudioZoneReport(data, reply);
            break;
        case static_cast<uint32_t>(AudioPolicyManagerInterfaceCode::ENABLE_AUDIO_ZONE_CHANGE_REPORT):
            HandleEnableAudioZoneChangeReport(data, reply);
            break;
        case static_cast<uint32_t>(AudioPolicyManagerInterfaceCode::ADD_UID_TO_AUDIO_ZONE):
            HandleAddUidToAudioZone(data, reply);
            break;
        case static_cast<uint32_t>(AudioPolicyManagerInterfaceCode::REMOVE_UID_FROM_AUDIO_ZONE):
            HandleRemoveUidFromAudioZone(data, reply);
            break;
        case static_cast<uint32_t>(AudioPolicyManagerInterfaceCode::ENABLE_SYSTEM_VOLUME_PROXY):
            HandleEnableSystemVolumeForZone(data, reply);
            break;
        case static_cast<uint32_t>(AudioPolicyManagerInterfaceCode::SET_SYSTEM_VOLUME_LEVEL_FOR_ZONE):
    ...
    uint32_t code, MessageParcel &data, MessageParcel &reply, MessageOption &option)
{
    switch (code) {
        case static_cast<uint32_t>(AudioPolicyManagerInterfaceCode::GET_AUDIO_INTERRUPT_FOR_ZONE):
            ...
            break;
        case static_cast<uint32_t>(AudioPolicyManagerInterfaceCode::GET_AUDIO_INTERRUPT_OF_DEVICE_FOR_ZONE):
            HandleGetAudioInterruptForZoneDevice(data, reply);
            break;
        case static_cast<uint32_t>(AudioPolicyManagerInterfaceCode::ENABLE_AUDIO_ZONE_INTERRUPT_REPORT):
            HandleEnableAudioZoneInterruptReport(data, reply);
            break;
        case static_cast<uint32_t>(AudioPolicyManagerInterfaceCode::INJECT_INTERRUPT_TO_AUDIO_ZONE):
            HandleInjectInterruptToAudioZone(data, reply);
            break;
        case static_cast<uint32_t>(AudioPolicyManagerInterfaceCode::INJECT_INTERRUPT_OF_DEVICE_TO_AUDIO_ZONE):
...
void AudioPolicyManagerStub::HandleGetAudioZone(MessageParcel &data, MessageParcel &reply)
{
    ...
    if (zoneDescriptor == nullptr) {
        reply.WriteInt32(ERR_NULL_NULLPTR);
    } else {
        reply.WriteInt32(SUCCESS);
