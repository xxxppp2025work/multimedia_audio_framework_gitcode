...
int32_t AudioPolicyServer::SetAppVolumeMuted(int32_t appUid, bool muted, int32_t volumeFlag = 0)
{
...
{
    AUDIO_ERR_LOG("zhangjinqi Test --- 4");
    if (!PermissionUtil::VerifySystemPermission()) {
        AUDIO_ERR_LOG("SetAppVolumeLevel: No system permission");
        return ERR_PERMISSION_DENIED;
    }
    // if (!IsAppVolumeLevelValid(appUid, volumeLevel)) {
    //     return ERR_NOT_SUPPORTED;
    // }
    std::lock_guard<std::mutex> lock(systemVolumeMutex_);
...
{
    AUDIO_INFO_LOG("SetSelfAppVolumeLevel volumeLevel: %{public}d, updateUi: %{public}d",
        volumeLevel, isUpdateUi);
...
bool AudioPolicyServer::IsAppVolumeMute(int32_t appUid, bool owned)
{
    ...
    int32_t ret = audioPolicyService_.IsAppVolumeMute(appUid, owned);
    CHECK_AND_RETURN_RET_LOG(ret == SUCCESS, ret, "Fail to get is App Volume mute");
}
