    ...
    uint32_t clientUid, const int32_t zoneID)
{
    ...
    uid_t callingUid = static_cast<uint32_t>(IPCSkeleton::GetCallingUid());
    if (callingUid != MEDIA_SERVICE_UID && callingUid !=  MCU_UID) {
        // if the callingUid is not media or mcu, it is necessary to verify that the parameters are valid.
        if (callingUid != clientUid) {
            AUDIO_ERR_LOG("The callingUid is not equal to clientUid and is not MEDIA_SERVICE_UID!");
            return ERR_UNKNOWN;
        }
        if (!audioPolicyService_.IsStreamBelongToUid(callingUid, sessionID)) {
