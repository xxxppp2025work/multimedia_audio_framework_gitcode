...
int32_t AudioPolicyServer::SetMicrophoneMuteAudioConfig(bool isMute)
{
    ...
    PrivacyKit::SetMutePolicy(POLICY_TYPE_MAP[TEMPORARY_POLCIY_TYPE], MICPHONE_CALLER, isMute, IPCSkeleton::GetCallingTokenID());
    guard.CheckCurrTimeout();
...
int32_t AudioPolicyServer::SetMicrophoneMutePersistent(const bool isMute, const PolicyType type)
{
    ...
    int32_t ret = PrivacyKit::SetMutePolicy(POLICY_TYPE_MAP[type], MICPHONE_CALLER, isMute, IPCSkeleton::GetCallingTokenID());
    guard.CheckCurrTimeout();
    if (ret != SUCCESS) {
