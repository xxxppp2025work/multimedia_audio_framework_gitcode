...
int32_t AudioCoreService::EventEntry::SetDefaultOutputDevice(const DeviceType deviceType, const uint32_t sessionID,
    const StreamUsage streamUsage, bool isRunning, bool notSet)
{
    std::lock_guard<std::shared_mutex> lock(eventMutex_);
    ...
    int32_t ret = coreService_->SetDefaultOutputDevice(deviceType, sessionID, streamUsage, isRunning, notSet);
    return ret;
}
