...
int32_t AudioInjectorPolicy::MoveStream(uint32_t renderId, bool flag)
{
    ...
    if (flag) {
        ...
        int32_t ret = audioPolicyManager_.MoveSinkInputByIndexOrName(renderId, 1, moduleInfo_.name);
        ...
        if (pipeManager_->IsPipeAlive(oldInfo)) {
            ...
            int32_t ret = audioIOHandleMap_.OpenPortAndInsertIOHandle(portName, moduleInfo);
            CHECK_AND_RETURN_RET_LOG(ret == SUCCESS, ERROR, "open original port failed");
        }
        int32_t ret = audioPolicyManager_.MoveSinkInputByIndexOrName(renderId, 1, portName);
