...
int32_t AudioCoreService::FetchOutputDeviceAndRoute(std::string caller, const AudioStreamDeviceChangeReasonExt reason)
{
    ...
    for (auto &streamDesc : outputStreamDescs) {
        ...
        streamDesc->newDeviceDescs_ =
            audioRouterCenter_.FetchOutputDevices((streamUsage != StreamUsage::STREAM_USAGE_INVALID) ? streamUsage :
            streamDesc->rendererInfo_.streamUsage, GetRealUid(streamDesc), caller + "FetchOutputDeviceAndRoute");
