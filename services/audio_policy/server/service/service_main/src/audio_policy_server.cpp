    ...
    const int32_t keyType, const AudioStreamType &streamInFocus)
{
    ...
    bool isHolding = false;
    FunctionHoldType funcHoldType = FUNCTION_HOLD_INVALID;
    if (!FindLoudVolStreamTypeEnable(streamInFocus, funcHoldType)) return false;
...
int32_t AudioPolicyServer::IsAppVolumeMute(int32_t appUid, bool owned, bool &isMute)
{
    ...
    if (isMute) {
        ReloadLoudVolumeMode(STREAM_APP, LOUD_VOLUME_SWITCH_OFF);
    }
    ...
    bool isUpdateUi, int32_t zoneId)
{
    ...
    int32_t volumeLevelMax = -1;
    int32_t streamInFocusInt = static_cast<int32_t>(streamType);
    GetMaxVolumeLevel(streamInFocusInt, volumeLevelMax);
    if (isUpdateUi == false && volumeLevelMax <= volumeLevel &&
        audioActiveDevice_.GetCurrentOutputDeviceType() == DeviceType::DEVICE_TYPE_SPEAKER &&
        CheckLoudVolumeMode(volumeLevel, OHOS::MMI::KeyEvent::KEYCODE_VOLUME_UP, streamType)) {
        AUDIO_INFO_LOG("device %{public}d, stream %{public}d, loud volume mode operation",
            audioActiveDevice_.GetCurrentOutputDeviceType(), streamType);
    } else if (isUpdateUi == false && volumeLevelMax > volumeLevel &&
        audioActiveDevice_.GetCurrentOutputDeviceType() == DeviceType::DEVICE_TYPE_SPEAKER &&
        CheckLoudVolumeMode(volumeLevel, OHOS::MMI::KeyEvent::KEYCODE_VOLUME_DOWN, streamType)) {
        AUDIO_INFO_LOG("device %{public}d, stream %{public}d, loud volume mode operation",
            audioActiveDevice_.GetCurrentOutputDeviceType(), streamType);
    }
    if (streamType == STREAM_ALL) {
