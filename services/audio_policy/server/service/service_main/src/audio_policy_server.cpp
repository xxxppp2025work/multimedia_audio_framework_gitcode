...
int32_t AudioPolicyServer::AddStreamUsageToAudioZone(int32_t zoneId, AudioStreamUsage usage)
{
    CHECK_AND_RETURN_RET_LOG(PermissionUtil::VerifySystemPermission(), ERR_PERMISSION_DENIED, "no system permission");
    return AudioZoneService::GetInstance().AddStreamUsageToAudioZone(zoneId, usage);
}

int32_t AudioPolicyServer::RemoveStreamUsageFromAudioZone(int32_t zoneId, AudioStreamUsage usage)
{
    CHECK_AND_RETURN_RET_LOG(PermissionUtil::VerifySystemPermission(), ERR_PERMISSION_DENIED, "no system permission");
    return AudioZoneService::GetInstance().RemoveStreamUsageFromAudioZone(zoneId, usage);
}
