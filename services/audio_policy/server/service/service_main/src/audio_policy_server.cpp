    ...
    bool mute, const bool& isUpdateUi, int32_t zoneId)
{
    ...
    if (volumeLevel == 0 && !mute) {
        muteStatus = true;
        audioPolicyService_.SetStreamMute(streamType, true, STREAM_USAGE_UNKNOWN, DEVICE_TYPE_NONE, zoneId);
    ...
    if (VolumeUtils::IsPCVolumeEnable()) {
        ...
        if (VolumeUtils::GetVolumeTypeFromStreamType(streamType) == STREAM_MUSIC &&
            ...
            audioPolicyService_.SetStreamMute(STREAM_SYSTEM, muteStatus, STREAM_USAGE_UNKNOWN, DEVICE_TYPE_NONE, zoneId);
