...
void AudioZoneService::MoveDeviceToGlobalFromZones(std::shared_ptr<AudioDeviceDescriptor> device)
{
    ...
    {
        std::lock_guard<std::mutex> lock(zoneMutex_);
        for (const auto &zoneMap : zoneMaps_) {
            CHECK_AND_CONTINUE_LOG(zoneMap.second != nullptr, "zone is nullptr");
            auto zoneDes = zoneMap.second->GetDescriptor();
            CHECK_AND_CONTINUE_LOG(zoneDes != nullptr, "zoneDes is nullptr");
            auto zone = FindZone(zoneDes->zoneId_);
            CHECK_AND_CONTINUE_LOG(zone != nullptr, "zone id %{public}d is not found", zoneId);
