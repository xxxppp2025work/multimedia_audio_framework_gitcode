    ...
    std::unordered_map<std::string, std::shared_ptr<AdapterDeviceInfo>> &tmpDeviceInfoMap_)
{
    if (pipeInfo_->streamPropInfos_.size() == 0) {
        ...
        for (auto deviceName : pipeInfo_->dynamicSupportDevices_) {
            auto uniqueDeviceIt = tmpDeviceInfoMap_.find(deviceName);
            if (uniqueDeviceIt == tmpDeviceInfoMap_.end()) {
                AUDIO_WARNING_LOG("pipe needed device:%{public}s not exists.", deviceName.c_str());
                continue;
            }
            pipeInfo_->dynamicSupportDeviceMap_.insert({uniqueDeviceIt->second->type_, uniqueDeviceIt->second});
...
void AudioPolicyConfigData::UpdateDynamicStreamProps(const std::string adapterName, const std::string &pipeName,
    std::list<std::shared_ptr<PipeStreamPropInfo>> &streamProps, const std::list<std::string> &supportDevices)
{
    ...
    for (auto &streamProp : streamProps) {
        ...
        for (auto deviceName : supportDevices) {
            ...
            if (uniqueDeviceIt == tmpDeviceInfoMap.end()) {
                AUDIO_WARNING_LOG("streamProp needed device %{public}s not exist", deviceName.c_str());
                continue;
            }
...
void AdapterPipeInfo::UpdateDynamicStreamProps(const std::list<std::shared_ptr<PipeStreamPropInfo>> &streamProps)
{
    ...
    dynamicSupportDevices_ = streamProps.front()->supportDevices_;
    dynamicSupportDeviceMap_ = streamProps.front()->supportDeviceMap_;
}
...
void AdapterPipeInfo::ClearDynamicStreamProps()
{
    ...
    dynamicSupportDevices_.clear();
    dynamicSupportDeviceMap_.clear();
}
