...
/**
 * @tc.name  : Test AudioCapturerSession.
 * @tc.number: FindRunningNormalSession_002
 * @tc.desc  : Test info->capturerState != CAPTURER_RUNNING || static_cast<uint32_t>(info->sessionId) == sessionId ||
 *             specialSourceTypeSet_.count(tmpSource) != 0
 */
HWTEST_F(AudioCapturerSessionExtTest, FindRunningNormalSession_002, TestSize.Level4)
{
    int32_t sessionId1 = 1001;
    int32_t sessionId2 = 1002;
    AudioStreamCollector &streamCollector = AudioStreamCollector::GetAudioStreamCollector();
    auto info = std::make_shared<AudioCapturerChangeInfo>();
    info->clientUID = THP_EXTRA_SA_UID + 1;
    info->sessionId = sessionId1;
    info->capturerState = CAPTURER_INVALID;
    streamCollector.audioCapturerChangeInfos_ = { info };

    SessionInfo sessionInfo;
    sessionInfo.sourceType = SOURCE_TYPE_INVALID;
    audioCapturerSession_->sessionWithNormalSourceType_.insert({ sessionId1, sessionInfo });
    AudioCapturerChangeInfo runningSessionInfo;
    runningSessionInfo.capturerInfo.sourceType = SOURCE_TYPE_INVALID;
    EXPECT_FALSE(audioCapturerSession_->FindRunningNormalSession(sessionId1, runningSessionInfo));
    EXPECT_FALSE(audioCapturerSession_->FindRunningNormalSession(sessionId2, runningSessionInfo));

    audioCapturerSession_->sessionWithNormalSourceType_[sessionId1].sourceType = SOURCE_TYPE_PLAYBACK_CAPTURE;
    EXPECT_FALSE(audioCapturerSession_->FindRunningNormalSession(sessionId1, runningSessionInfo));
    EXPECT_FALSE(audioCapturerSession_->FindRunningNormalSession(sessionId2, runningSessionInfo));
    ...
    audioCapturerSession_->sessionWithNormalSourceType_[sessionId1].sourceType = SOURCE_TYPE_INVALID;
    EXPECT_FALSE(audioCapturerSession_->FindRunningNormalSession(sessionId1, runningSessionInfo));
    EXPECT_FALSE(audioCapturerSession_->FindRunningNormalSession(sessionId2, runningSessionInfo));
}
...
/**
 * @tc.name  : Test AudioCapturerSession.
 * @tc.number: FindRunningNormalSession_003
 * @tc.desc  : Test IsHigherPrioritySourceType(tmpSource, runningSessionInfo.capturerInfo.sourceType)
 */
HWTEST_F(AudioCapturerSessionExtTest, FindRunningNormalSession_003, TestSize.Level4)
{
    AudioStreamCollector &streamCollector = AudioStreamCollector::GetAudioStreamCollector();
    auto info = std::make_shared<AudioCapturerChangeInfo>();
    info->clientUID = THP_EXTRA_SA_UID + 1;
    ...
    audioCapturerSession_->sessionWithNormalSourceType_.insert({ info->sessionId, sessionInfo });
    AudioEcManager::GetInstance().isEcFeatureEnable_ = true;
    AudioCapturerChangeInfo runningSessionInfo;
    runningSessionInfo.capturerInfo.sourceType = SOURCE_TYPE_VOICE_COMMUNICATION;
    uint32_t sessionId = 1002;
    // IsHigherPrioritySourceType() == true
    EXPECT_TRUE(audioCapturerSession_->FindRunningNormalSession(sessionId, runningSessionInfo));

    // IsHigherPrioritySourceType() == false
    runningSessionInfo.capturerInfo.sourceType = SOURCE_TYPE_VOICE_CALL;
    EXPECT_FALSE(audioCapturerSession_->FindRunningNormalSession(sessionId, runningSessionInfo));
    runningSessionInfo.capturerInfo.sourceType = SOURCE_TYPE_VIRTUAL_CAPTURE;
    EXPECT_FALSE(audioCapturerSession_->FindRunningNormalSession(sessionId, runningSessionInfo));
    audioCapturerSession_->sessionWithNormalSourceType_[info->sessionId].sourceType = SOURCE_TYPE_EC;
    EXPECT_FALSE(audioCapturerSession_->FindRunningNormalSession(sessionId, runningSessionInfo));
    AudioEcManager::GetInstance().isEcFeatureEnable_ = false;
    EXPECT_FALSE(audioCapturerSession_->FindRunningNormalSession(sessionId, runningSessionInfo));
}
