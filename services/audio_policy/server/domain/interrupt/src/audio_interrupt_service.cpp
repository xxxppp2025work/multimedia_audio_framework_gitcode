...
void AudioInterruptService::HandleAppStreamType(AudioInterrupt &audioInterrupt, const int32_t zoneId)
{
    ...
    const int32_t zoneId, const int32_t callerPid, bool &updateScene)
{
    for (const auto &itZone : zoneMap_) {
        CHECK_AND_CONTINUE_LOG((itZone->second != nullptr), "itZone or itZone->second is nullptr");
        std::list<std::pair<AudioInterrupt, AudioFocuState>> audioFocusInfoList = itZone.second->audioFocusInfoList;
        ...
        */
        auto isStreamFocusPresent = [&](const std::pair<AudioInterrupt, AudioFocuState> &pair) {
            CHECK_AND_RETURN_RET_LOG(sessionService_ != nullptr, false,
                "sessionService_ is nullptr");
        ...
        for (const auto &it : audioFocusInfoList) {
            CHECK_AND_CONTINUE_LOG(isStreamFocusPresent(it), "isStreamFocusPresent is false");
            updateScene = true;
            int32_t ret = ActivateAudioInterruptCoreProcedure(zoneId, it.first, true, tempUpdateScene);
            CHECK_AND_RETURN_RET_LOG(ret == SUCCESS, ret, "ActivateAudioInterruptCoreProcedure fail");
        }
    }
