...
std::shared_ptr<AudioInterruptService> AudioInterruptService::audioInterruptService_ = nullptr;
std::mutex AudioInterruptService::createInstMutx_;
...
static std::shared_ptr<AudioInterruptService> AudioInterruptService::GetInstance()
{
    if (audioInterruptService_ == nullptr) {
        std::lock_guard<std::mutex> lock(createInstMutx_);
        if (audioInterruptService_ == nullptr) {
            audioInterruptService_ = std::shared_ptr<AudioInterruptService>(new AudioInterruptService());
        }
    }

    return audioInterruptService_;
}
...
{
    if (HasAudioSessionFakeInterrupt(ZONEID_DEFAULT, pid)) {
...
int32_t AudioInterruptService::DeactivateAudioSession(const int32_t zoneId, const int32_t callerPid)
{
    ...
    if (HasAudioSessionFakeInterrupt(zoneId, callerPid)) {
        ...
        if (handler_ != nullptr) {
            ...
            auto streamsInSession = sessionService_.GetStreams(callerPid);
            InterruptEventInternal interruptEvent {INTERRUPT_TYPE_BEGIN, INTERRUPT_FORCE, INTERRUPT_HINT_STOP, 1.0f};
...
void AudioInterruptService::DeactivateAudioSessionInFakeFocusMode(const int32_t pid, InterruptHint hintType)
{
    ...
    */
    auto streamsInSession = sessionService_.GetStreams(pid);
...
bool AudioInterruptService::IsStreamAllowedToSetDevice(const uint32_t streamId)
{
    std::lock_guard<std::mutex> lock(mutex_);
    return sessionService_.IsStreamAllowedToSetDevice(streamId);
}
...
AudioStreamType AudioInterruptService::GetStreamInFocusInternal(const int32_t uid, const int32_t zoneId)
{
    ...
    for (auto iter = audioFocusInfoList.begin(); iter != audioFocusInfoList.end(); ++iter) {
        ...
        if (iter->first.isAudioSessionInterrupt) {
            auto sessionStreams = sessionService_.GetStreams(iter->first.pid);
...
AudioScene AudioInterruptService::GetHighestPriorityAudioScene(const int32_t zoneId)
{
    ...
    for (const auto&[interrupt, focuState] : audioFocusInfoList) {
        ...
        if (interrupt.isAudioSessionInterrupt) {
            auto streamsInSession = sessionService_.GetStreams(interrupt.pid);
            for (auto &it : streamsInSession) {
    ...
    const InterruptEventInternal &interruptEvent, const AudioInterrupt &audioInterrupt)
{
    ...
    */
    if (sessionService_.ShouldAudioStreamProcessHintType(interruptEvent.hintType)) {
        auto &audioInterrupts = sessionService_.GetStreams(audioInterrupt.pid);
    ...
    InterruptEventInternal &interruptEvent, const AudioInterrupt &audioInterrupt)
{
    ...
    auto sessionStreams = sessionService_.GetStreams(audioInterrupt.pid);
