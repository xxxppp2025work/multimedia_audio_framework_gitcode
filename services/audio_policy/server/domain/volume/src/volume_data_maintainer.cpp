    ...
    bool muteStatus)
{
    ...
    if (!muteKey.compare("")) {
        WriteVolumeDbAccessExceptionEvent(static_cast<int32_t>(VolumeDbAccessExceptionFuncId::SAVE_MUTE_STATUS_INTERNAL),
            ERR_READ_FAILED);
...
bool VolumeDataMaintainer::GetMuteStatusInternal(DeviceType deviceType, AudioStreamType streamType)
{
    std::string muteKey = GetMuteKeyForDataShare(deviceType, streamType);
    if (!muteKey.compare("")) {
        WriteVolumeDbAccessExceptionEvent(static_cast<int32_t>(VolumeDbAccessExceptionFuncId::GET_MUTE_STATUS_INTERNAL_A),
            ERR_READ_FAILED);
        AUDIO_ERR_LOG("[device %{public}d, streamType %{public}d] is not supported for "\
            "datashare", deviceType, streamType);
        return false;
    ...
    if (ret != SUCCESS) {
        WriteVolumeDbAccessExceptionEvent(static_cast<int32_t>(VolumeDbAccessExceptionFuncId::GET_MUTE_STATUS_INTERNAL_B),
            static_cast<int32_t>(ret));
...
bool VolumeDataMaintainer::SaveMuteTransferStatus(bool status)
{
    ...
    if (ret != SUCCESS) {
        WriteVolumeDbAccessExceptionEvent(static_cast<int32_t>(VolumeDbAccessExceptionFuncId::SAVE_MUTE_TRANSFER_STATUS),
            static_cast<int32_t>(ret));
...
bool VolumeDataMaintainer::GetSafeVolumeTime(DeviceType deviceType, int64_t &time)
{
    ...
    switch (deviceType) {
        ...
        case DEVICE_TYPE_USB_ARM_HEADSET:
            ...
            break;
        default:
            WriteVolumeDbAccessExceptionEvent(static_cast<int32_t>(VolumeDbAccessExceptionFuncId::GET_SAFE_VOLUME_TIME_A),
                static_cast<int32_t>(ret));
...
bool VolumeDataMaintainer::SetRestoreVolumeLevel(DeviceType deviceType, int32_t volume)
{
    ...
    if (ret != SUCCESS) {
        WriteVolumeDbAccessExceptionEvent(static_cast<int32_t>(VolumeDbAccessExceptionFuncId::SET_RESTORE_VOLUME_LEVEL_B),
            static_cast<int32_t>(ret));
        AUDIO_ERR_LOG("device:%{public}d, insert failed", deviceType);
        return false;
    }
...
bool VolumeDataMaintainer::GetRestoreVolumeLevel(DeviceType deviceType, int32_t &volume)
{
    ...
    if (ret != SUCCESS) {
        WriteVolumeDbAccessExceptionEvent(static_cast<int32_t>(VolumeDbAccessExceptionFuncId::GET_RESTORE_VOLUME_LEVEL_B),
            static_cast<int32_t>(ret));
        AUDIO_ERR_LOG("device:%{public}d, insert failed", deviceType);
        return false;
    }
