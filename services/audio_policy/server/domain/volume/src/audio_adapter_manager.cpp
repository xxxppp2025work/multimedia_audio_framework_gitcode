...
static std::unordered_map<DeviceType, std::vector<std::string>> DEVICE_CLASS_MAP = {
...
int32_t AudioAdapterManager::SetAdjustVolumeForZone(int32_t zoneId)
{
    ...
    if (zoneId > 0 ) {
        ...
        if (volumeDataMaintainer_.find(devices[0]->deviceType_) == volumeDataMaintainer_.end()) {
            volumeDataExtMaintainer_[devices[0]->deviceType_] = std::make_shared<VolumeDataMaintainer>();
            // 第一次链接的时候使得音量值最大
            for (auto &d : defaultVolumeTypeList_) {
                volumeDataExtMaintainer_[devices[0]->deviceType_]->muteStatusMap_[d] = false;
                volumeDataExtMaintainer_[devices[0]->deviceType_]->volumeLevelMap_[d] = MAX_VOLUME_LEVEL;
            }
            // 使得音量生效
            auto iter = defaultVolumeTypeList_.begin();
            while (iter != defaultVolumeTypeList_.end()) {
...
{   
    std::vector<std::shared_ptr<AudioDeviceDescriptor>> devices = AudioZoneService::GetInstance().FetchOutputDevices(
        zoneId, STREAM_USAGE_UNKNOWN, 0, ROUTER_TYPE_DEFAULT);
    CHECK_AND_RETURN_RET_LOG(devices.size() >= 1, ERR_OPERATION_FAILED,
        "devices size invalid");
...
{
    int32_t volumeLevel =
        volumeDataExtMaintainer_[devices[0]->deviceType_]->GetStreamVolume(streamType) * 
...
void AudioAdapterManager::SetAudioVolume(std::shared_ptr<AudioDeviceDescriptor> &device, AudioStreamType streamType, float volumeDb)
{
    ...
    int32_t volumeLevel = volumeDataExtMaintainer_[devices[0]->deviceType_]->GetStreamVolume(volumeType) * (isMuted ? 0 : 1);
    auto audioVolume = AudioVolume::GetInstance();
    CHECK_AND_RETURN_LOG(audioVolume != nullptr, "audioVolume handle null");
    ...
    StreamUsage streamUsage, const DeviceType &deviceType)
{
    ...
    volumeDataExtMaintainer_[devices[0]->deviceType_]->SetStreamMuteStatus(streamType, mute);
...
void AudioAdapterManager::SetVolumeForSwitchDevice(AudioDeviceDescriptor deviceDescriptor)
{
    ...
    // can not use this interface when device not in global device lists
...
bool AudioAdapterManager::LoadVolumeMap(std::shared_ptr<AudioDeviceDescriptor> &device)
{
    bool result = false;
    for (auto &streamType: defaultVolumeTypeList_) {
        result = volumeDataExtMaintainer_[devices[0]->deviceType_]->GetVolume(device->deviceType_, streamType);
        if (!result) {
            AUDIO_ERR_LOG("LoadVolumeMap: Could not load volume for streamType[%{public}d] from kvStore", streamType);
        }
    }
