...
int32_t AudioAdapterManager::SetQueryClientTypeCallback(const sptr<IRemoteObject> &object)
{
    lock_guard<mutex>(g_deviceVolumeBehaviorListenerMutex);
    deviceVolumeBehaviorListener_ = iface_cast<IStandardAudioPolicyManagerListener>(object);
    return SUCCESS;
}

void AudioAdapterManager::HandleDistributeionVolume()
{
    if (!currentActiveDevice_.volumeBehavior_.isReady) {
        ...
        if (!currentActiveDevice_.volumeBehavior_.isReady) {
            CHECK_AND_RETURN(deviceVolumeBehaviorListener_ != nullptr, "deviceVolumeBehaviorListener_ is nullptr!");
