    ...
    DeviceStatusListener * devListener)
{
    if (serviceStatus->status == SERVIE_STATUS_START) {
        ...
        if (statusInfo.isConnected) {
            ...
            // TODO: parse supportDevices(or set by us?)
            for (auto i = 0; i < deviceStreamInfoList.size(); i++) {
                supportDevices.push_back("Cast_Out"); // check: supportdevices name in xml? only one device?
            }
            AudioPolicyConfigManager::GetInstance().UpdateStreamPropInfo("remote", "offload_distributed_output",
                deviceStreamInfoList, supportDevices); // TODO: adapterName? pipeName?supportDevices?
            // TODO: update audioDeviceDescriptor
        } else {
            // TODO: adapterName? pipeName?
