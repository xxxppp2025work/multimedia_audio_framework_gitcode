...
#include "audio_stream_id_collector.h"
...
const uint32_t FIRST_SESSIONID = 1000000;
constexpr uint32_t MAX_VALID_SESSIONID = UINT32_MAX - FIRST_SESSIONID;
static uint32_t g_sessionId = FIRST_SESSIONID;

uint32_t AudioStreamIdCollector::GenerateStreamId()
{
    std::lock_guard<std::mutex> lock(sessionIdAllocatoeMutex_);
    uint32_t sessionId = g_sessionId++;
    if (sessionId > MAX_VALID_SESSIONID) {
