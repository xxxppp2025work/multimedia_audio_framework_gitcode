...
* @tc.desc  : Test IsRingerOverPlayback interface.
*/
HWTEST_F(AudioDeviceCommonUnitTest, AudioDeviceCommon_091, TestSize.Level1)
{
    ...
    EXPECT_FALSE(audioDeviceCommon.IsRingerOverPlayback(mode, state));

    mode = AUDIO_MODE_PLAYBACK;
    EXPECT_FALSE(audioDeviceCommon.IsRingerOverPlayback(mode, state));

    state = RENDERER_STOPPED;
    EXPECT_TRUE(audioDeviceCommon.IsRingerOverPlayback(mode, state));
