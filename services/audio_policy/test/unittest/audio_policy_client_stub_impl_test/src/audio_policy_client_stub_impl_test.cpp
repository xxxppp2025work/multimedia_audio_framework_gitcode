...
HWTEST(AudioPolicyClientStubImplTest, AudioPolicyClientStubImpl_019, TestSize.Level1)
{
    ...
    int32_t result = audioPolicyClient->AddPreferredOutputDeviceChangeCallback(cb);
    EXPECT_EQ(result, SUCCESS);

    std::vector<std::shared_ptr<AudioDeviceDescriptor>> desc;
    audioPolicyClient->OnPreferredOutputDeviceUpdated(desc);
    EXPECT_NE(audioPolicyClient, nullptr);
}
...
HWTEST(AudioPolicyClientStubImplTest, AudioPolicyClientStubImpl_020, TestSize.Level1)
{
    ...
    int32_t result = audioPolicyClient->AddPreferredInputDeviceChangeCallback(cb);
    EXPECT_EQ(result, SUCCESS);

    std::vector<std::shared_ptr<AudioDeviceDescriptor>> desc;
    audioPolicyClient->OnPreferredInputDeviceUpdated(desc);
    EXPECT_NE(audioPolicyClient, nullptr);
}
