...
HWTEST_F(AudioStreamCollectorUnitTest, HandleAppStateChange_001, TestSize.Level1)
{
    ...
    changeInfo->sessionId = 1;
    audioStreamCollector_.audioRendererChangeInfos_.push_back(std::move(changeInfo));
    int32_t result = audioStreamCollector_.HandleAppStateChange(clientUid, true, notifyMute);
    EXPECT_TRUE(changeInfo->backMute);
    result = audioStreamCollector_.HandleAppStateChange(clientUid, false, notifyMute);
...
HWTEST_F(AudioStreamCollectorUnitTest, HandleFreezeStateChange_001, TestSize.Level1)
{
    ...
    changeInfo->sessionId = 1;
    audioStreamCollector_.audioRendererChangeInfos_.push_back(std::move(changeInfo));
    int32_t result = audioStreamCollector_.HandleFreezeStateChange(clientPid, true, hasSession);
    EXPECT_TRUE(changeInfo->backMute);
    result = audioStreamCollector_.HandleFreezeStateChange(clientPid, false, hasSession);
    EXPECT_TRUE(changeInfo->backMute);
    hasSession = true;
    result = audioStreamCollector_.HandleFreezeStateChange(clientPid, false, hasSession);
    EXPECT_FALSE(changeInfo->backMute);
...
HWTEST_F(AudioStreamCollectorUnitTest, HandleBackTaskStateChange_001, TestSize.Level1)
{
    ...
    changeInfo->backMute = true;
    audioStreamCollector_.audioRendererChangeInfos_.push_back(std::move(changeInfo));
    int32_t result = audioStreamCollector_.HandleBackTaskStateChange(clientUid, hasSession);
    EXPECT_TRUE(changeInfo->backMute);
    hasSession = true;
    result = audioStreamCollector_.HandleBackTaskStateChange(clientUid, hasSession);
    EXPECT_FALSE(changeInfo->backMute);
}
...
HWTEST_F(AudioStreamCollectorUnitTest, HandleStartStreamMuteState_001, TestSize.Level1)
{
    ...
    changeInfo->sessionId = 1;
    audioStreamCollector_.audioRendererChangeInfos_.push_back(std::move(changeInfo));
    int32_t result = audioStreamCollector_.HandleStartStreamMuteState(clientUid, true);
    EXPECT_TRUE(changeInfo->backMute);
    result = audioStreamCollector_.HandleStartStreamMuteState(clientUid, false);
    EXPECT_FALSE(changeInfo->backMute);
}
} // namespace AudioStandard
} // namespace OHOS
