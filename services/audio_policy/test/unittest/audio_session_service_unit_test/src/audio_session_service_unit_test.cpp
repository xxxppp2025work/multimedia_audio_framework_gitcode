...
public:
    ...
    MOCK_METHOD(void, OnSessionTimeout, (const int32_t pid), (override));
};
...
HWTEST_F(AudioSessionServiceUnitTest, AudioSessionServiceUnitTest_010, TestSize.Level1)
{
    ...
    int32_t callerPid = 1;
    AudioSessionStrategy strategy;
    sessionService->sessionMap_.reserve(1001);
    for (int32_t i = 0; i < 1001; ++i) {
        sessionService->sessionMap_.emplace(i, std::make_shared<AudioSession>(callerPid, strategy, nullptr));
    }
...
HWTEST_F(AudioSessionServiceUnitTest, AudioSessionServiceUnitTest_012, TestSize.Level1)
{
    ...
    EXPECT_CALL(*mockTimeOutCallback, OnSessionTimeout(_)).Times(1);
...
HWTEST_F(AudioSessionServiceUnitTest, AudioSessionServiceUnitTest_016, TestSize.Level1)
{
    ...
    EXPECT_EQ(fakeAudioInterrupt.streamId, START_FAKE_STREAM_ID);
