...
/**
* @tc.name  : Test AudioVolumeManager.
* @tc.number: AudioVolumeManager_VolumeLimit_002
* @tc.desc  : Test reset volume limit interface.
*/
HWTEST_F(AudioVolumeManagerUnitTest, AudioVolumeManager_VolumeLimit_002, TestSize.Level1)
{
    auto audioVolumeManager = std::make_shared<AudioVolumeManager>();
    ASSERT_TRUE(audioVolumeManager != nullptr);

    AudioDeviceDescriptor audioDeviceDescriptor;
    audioVolumeManager->audioSceneManager_.SetAudioScenePre(AUDIO_SCENE_PHONE_CALL);
    int32_t ret = audioVolumeManager->SetVolumeForSwitchDevice(audioDeviceDescriptor, PORT_NONE, false);
    EXPECT_EQ(ret, SUCCESS);
    audioVolumeManager->audioSceneManager_.SetAudioScenePre(AUDIO_SCENE_DEFAULT);
    ret = audioVolumeManager->SetVolumeForSwitchDevice(audioDeviceDescriptor, PORT_NONE, false);
    EXPECT_EQ(ret, SUCCESS);
    auto &manager = static_cast<AudioAdapterManager &>(audioVolumeManager->audioPolicyManager_);
    float newLimit = manager.volumeLimit_.load();
    EXPECT_EQ(newLimit, 1.0f);
}
} // namespace AudioStandard
} // namespace OHOS
