...
HWTEST(VolumeDataMaintainerUnitTest, GetVolumeWithDatabaseVolumeName_001, TestSize.Level1)
{
    ...
    // Test setting mute
    volumeDataMaintainer->SetAppStreamMutd(appUid, streamType, true);
    EXPECT_EQ(VolumeDataMaintainer->IsAppStreamMuted(appUid, streamType), true);

    // Test setting unmute
    volumeDataMaintainer->SetAppStreamMutd(appUid, streamType, false);
    ...
    volumeDataMaintainer->SetAppStreamMutd(anotherAppUid, streamType, true);
    ...
    volumeDataMaintainer->SetAppStreamMutd(appUid, anotherStreamType, true);
    EXPECT_EQ(VolumeDataMaintainer->IsAppStreamMuted(appUid, anotherStreamType), true);
    EXPECT_EQ(VolumeDataMaintainer->IsAppStreamMuted(appUid, streamType), true);

    // Test delete an app entry
    volumeDataMaintainer->SetAppStreamMutd(appUid, streamType, false);
    volumeDataMaintainer->SetAppStreamMutd(appUid, anotherStreamType, false);
    EXPECT_EQ(VolumeDataMaintainer->IsAppStreamMuted(appUid, streamType), false);
