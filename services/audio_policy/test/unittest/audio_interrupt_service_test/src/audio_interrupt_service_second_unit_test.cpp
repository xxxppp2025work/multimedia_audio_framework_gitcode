...
* @tc.desc  : Test IsAudioSessionActivated、SetSessionDefaultOutputDevice
*/
HWTEST(AudioInterruptServiceSecondUnitTest, AudioInterruptService_007, TestSize.Level1)
{
    auto audioInterruptService = std::make_shared<AudioInterruptService>();
    ASSERT_NE(audioInterruptService, nullptr);
    ...
    InterruptHint hintType = INTERRUPT_HINT_PAUSE;
    EXPECT_EQ(nullptr, audioInterruptService->sessionService_);
    auto ret = audioInterruptService->IsAudioSessionActivated(pid);
    EXPECT_EQ(false, ret);

    audioInterruptService->sessionService_ = nullptr;
    AudioPolicyConfigManager::GetInstance().hasEarpiece_ = true;
    auto result = audioInterruptService->SetSessionDefaultOutputDevice(pid, DEVICE_TYPE_SPEAKER);
    EXPECT_EQ(ERR_UNKNOWN, result);
    auto audioInterruptZone = make_shared<AudioInterruptZone>();
    AudioInterrupt audioInterrupt;
    audioInterrupt.pid = pid;
    audioInterrupt.isAudioSessionInterrupt = true;
    ...
    ret = audioInterruptService->IsAudioSessionActivated(pid);
    EXPECT_EQ(false, ret);
    ret = audioInterruptService->SetSessionDefaultOutputDevice(pid, DEVICE_TYPE_SPEAKER);
    EXPECT_EQ(false, ret);
}
