...
*/
HWTEST(AudioInterruptUnitTest, AudioInterruptServiceAddActiveInterruptToSession001, TestSize.Level1)
{
    auto interruptService = GetTnterruptServiceTest();
    interruptService->sessionService_ = nullptr;
    int32_t pid = CALLER_PID;
...
HWTEST(AudioInterruptUnitTest, AudioInterruptServiceAddActiveInterruptToSession_003, TestSize.Level1)
{
    ...
    AudioInterrupt audioInterrupt;
    audioInterrupt.sessionId = CALLER_PID;
...
/**
 * @tc.name  : Test AudioInterruptService.
 * @tc.number: AudioInterruptService_UpdateAudioSceneFromInterrupt_001
 * @tc.desc  : Test UpdateAudioSceneFromInterrupt.
 */
HWTEST(AudioInterruptUnitTest, AudioInterruptService_UpdateAudioSceneFromInterrupt_001, TestSize.Level1)
{
    auto interruptServiceTest = GetTnterruptServiceTest();
    interruptServiceTest->zonesMap_.clear();

    interruptServiceTest->Init(GetPolicyServerTest());
    AudioInterruptChangeType illegalChangeType = static_cast<AudioInterruptChangeType>(2);
    interruptServiceTest->UpdateAudioSceneFromInterrupt(AUDIO_SCENE_DEFAULT, illegalChangeType);
    EXPECT_NE(ACTIVATE_AUDIO_INTERRUPT, illegalChangeType);
    EXPECT_NE(DEACTIVATE_AUDIO_INTERRUPT, illegalChangeType);

    interruptServiceTest->policyServer_ = nullptr;
    interruptServiceTest->UpdateAudioSceneFromInterrupt(AUDIO_SCENE_INVALID,
        AudioInterruptChangeType::ACTIVATE_AUDIO_INTERRUPT);
    EXPECT_EQ(nullptr, interruptServiceTest->policyServer_);
...
/**
 * @tc.name  : Test AudioInterruptService.
 * @tc.number: AudioInterruptService_ResumeAudioFocusList_001
 * @tc.desc  : Test ResumeAudioFocusList.
 */
HWTEST(AudioInterruptUnitTest, AudioInterruptService_ResumeAudioFocusList_001, TestSize.Level1)
{
    auto interruptServiceTest = GetTnterruptServiceTest();
    interruptServiceTest->zonesMap_.clear();

    interruptServiceTest->ResumeAudioFocusList(0, true);
    EXPECT_TRUE(interruptServiceTest->zonesMap_.empty());
}

/**
 * @tc.name  : Test AudioInterruptService.
 * @tc.number: AudioInterruptService_ResumeAudioFocusList_002
 * @tc.desc  : Test ResumeAudioFocusList.
 */
HWTEST(AudioInterruptUnitTest, AudioInterruptService_ResumeAudioFocusList_002, TestSize.Level1)
{
    auto interruptServiceTest = GetTnterruptServiceTest();
    interruptServiceTest->zonesMap_.clear();
    ...
    interruptServiceTest->zonesMap_[0] = nullptr;
    interruptServiceTest->ResumeAudioFocusList(0, true);
    EXPECT_EQ(nullptr, interruptServiceTest->zonesMap_.find(0)->second);
}

/**
 * @tc.name  : Test AudioInterruptService.
 * @tc.number: AudioInterruptService_ResumeAudioFocusList_003
 * @tc.desc  : Test ResumeAudioFocusList.
 */
HWTEST(AudioInterruptUnitTest, AudioInterruptService_ResumeAudioFocusList_003, TestSize.Level1)
{
    auto interruptServiceTest = GetTnterruptServiceTest();
    interruptServiceTest->zonesMap_.clear();
    ...
    AudioInterrupt audioInterrupt;
    audioInterrupt.audioFocusType.streamType = STREAM_RING;
    interruptServiceTest->zonesMap_.find(0)->second->audioFocusInfoList.emplace_back(audioInterrupt, ACTIVE);

    interruptServiceTest->ResumeAudioFocusList(0, true);
    auto it = interruptServiceTest->zonesMap_.find(0)->second->audioFocusInfoList.begin();
    std::advance(it, 0);
    EXPECT_EQ(ACTIVE, it->second);
}

/**
 * @tc.name  : Test AudioInterruptService.
 * @tc.number: AudioInterruptService_ResumeAudioFocusList_004
 * @tc.desc  : Test ResumeAudioFocusList.
 */
HWTEST(AudioInterruptUnitTest, AudioInterruptService_ResumeAudioFocusList_004, TestSize.Level1)
{
    auto interruptServiceTest = GetTnterruptServiceTest();
    interruptServiceTest->zonesMap_.clear();
    interruptServiceTest->zonesMap_[0] = std::make_shared<AudioInterruptZone>();
    ...
    interruptServiceTest->zonesMap_.find(0)->second->audioFocusInfoList.emplace_back(audioInterrupt, PAUSE);

    interruptServiceTest->ResumeAudioFocusList(0, true);
    auto it = interruptServiceTest->zonesMap_.find(0)->second->audioFocusInfoList.begin();
    std::advance(it, 0);
    EXPECT_TRUE(interruptServiceTest->zonesMap_.find(0)->second->audioFocusInfoList.empty());
}

/**
 * @tc.name  : Test AudioInterruptService.
 * @tc.number: AudioInterruptService_ResumeAudioFocusList_005
 * @tc.desc  : Test ResumeAudioFocusList.
 */
HWTEST(AudioInterruptUnitTest, AudioInterruptService_ResumeAudioFocusList_005, TestSize.Level1)
{
    auto interruptServiceTest = GetTnterruptServiceTest();
    interruptServiceTest->zonesMap_.clear();
    interruptServiceTest->zonesMap_[0] = std::make_shared<AudioInterruptZone>();
    ...
    audioInterrupt.sessionId = 0;
    interruptServiceTest->zonesMap_.find(0)->second->audioFocusInfoList.emplace_back(audioInterrupt, PAUSE);
    interruptServiceTest->handler_ = std::make_shared<AudioPolicyServerHandler>();
...
/**
 * @tc.name  : Test AudioInterruptService.
 * @tc.number: AudioInterruptService_ResumeAudioFocusList_006
 * @tc.desc  : Test ResumeAudioFocusList.
 */
HWTEST(AudioInterruptUnitTest, AudioInterruptService_ResumeAudioFocusList_006, TestSize.Level1)
{
    auto interruptServiceTest = GetTnterruptServiceTest();
    interruptServiceTest->zonesMap_.clear();
    interruptServiceTest->zonesMap_[0] = std::make_shared<AudioInterruptZone>();
    AudioInterrupt audioInterrupt;
    audioInterrupt.audioFocusType.streamType = STREAM_RING;
    interruptServiceTest->zonesMap_.find(0)->second->audioFocusInfoList.emplace_back(audioInterrupt, DUCK);
    interruptServiceTest->handler_ = std::make_shared<AudioPolicyServerHandler>();

    interruptServiceTest->ResumeAudioFocusList(0, true);
    EXPECT_FALSE(interruptServiceTest->zonesMap_.find(0)->second->audioFocusInfoList.empty());
}

/**
 * @tc.name  : Test AudioInterruptService.
 * @tc.number: AudioInterruptService_ResumeAudioFocusList_007
 * @tc.desc  : Test ResumeAudioFocusList.
 */
HWTEST(AudioInterruptUnitTest, AudioInterruptService_ResumeAudioFocusList_007, TestSize.Level1)
{
    auto interruptServiceTest = GetTnterruptServiceTest();
    ...
    interruptServiceTest->zonesMap_[0] = std::make_shared<AudioInterruptZone>();
    AudioInterrupt audioInterrupt;
    audioInterrupt.mode = SHARE_MODE;
    audioInterrupt.audioFocusType.streamType = STREAM_RING;
    audioInterrupt.pid = 0;
    interruptServiceTest->zonesMap_.find(0)->second->audioFocusInfoList.emplace_back(audioInterrupt, PAUSE);
    ...
    std::pair<AudioFocusType, AudioFocusType> audioFocusTypePair =
        std::make_pair(audioInterrupt.audioFocusType, audioInterrupt.audioFocusType);
    AudioFocusEntry focusEntry;
    focusEntry.isReject = false;
    focusEntry.actionOn = CURRENT;
    focusEntry.hintType = INTERRUPT_HINT_PAUSE;
    interruptServiceTest->focusCfgMap_[audioFocusTypePair] = focusEntry;
...
/**
 * @tc.name  : Test AudioInterruptService.
 * @tc.number: AudioInterruptService_ResumeAudioFocusList_008
 * @tc.desc  : Test ResumeAudioFocusList.
 */
HWTEST(AudioInterruptUnitTest, AudioInterruptService_ResumeAudioFocusList_008s, TestSize.Level1)
{
    auto interruptServiceTest = GetTnterruptServiceTest();
    interruptServiceTest->focusCfgMap_.clear();
    interruptServiceTest->zonesMap_.clear();
