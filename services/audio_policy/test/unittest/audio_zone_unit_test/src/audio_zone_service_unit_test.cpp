...
HWTEST_F(AudioZoneServiceUnitTest, AudioZoneService_003, TestSize.Level1)
{
    ...
    EXPECT_EQ(AudioZoneService::GetInstance().InjectInterruptToAudioZone(0, "", interrupts), 0);
    EXPECT_EQ(AudioZoneService::GetInstance().InjectInterruptToAudioZone(0, "0", interrupts), 0);
}
} // namespace AudioStandard
} // namespace OHOS
