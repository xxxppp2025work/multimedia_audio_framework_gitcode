...
int32_t AudioCoreService::CreateRendererClient(std::shared_ptr<AudioStreamDescriptor> streamDesc, AudioFlag &audioFlag, uint32_t &sessionId)
{
...
int32_t AudioCoreService::CreateCapturerClient(std::shared_ptr<AudioStreamDescriptor> streamDesc, AudioFlag &audioFlag, uint32_t &sessionId)
{
    streamDesc->oldDeviceDescs_ = streamDesc->newDeviceDescs_;
    // Select device
    int32_t ret = SUCCESS;
    ...
    const AudioStreamDeviceChangeReasonExt reason)
{
    ...
    // std::shared_ptr<AudioStreamDescriptor> streamDesc = GetStreamDescById(streamChangeInfo.audioRendererChangeInfo.sessionId);
...
void AudioCoreService::FetchInputDeviceForTrack(AudioStreamChangeInfo &streamChangeInfo)
{
    ...
    // // Get streamDesc from pipeManager, then select deviceType and pipeType
    // std::shared_ptr<AudioStreamDescriptor> streamDesc = GetStreamDescById(streamChangeInfo.audioCapturerChangeInfo.sessionId);
...
int32_t AudioCoreService::SelectOutputDeviceAndRoute(const AudioStreamDeviceChangeReasonExt reason)
{
    ...
    std::vector<std::shared_ptr<AudioStreamDescriptor>> outputStreamDescs; // in plan: wait pipeMananger
    for (auto streamDesc : outputStreamDescs) {
        streamDesc->oldDeviceDescs_ = streamDesc->newDeviceDescs_;
        streamDesc->newDeviceDescs_ = audioRouterCenter_.FetchOutputDevices(streamDesc->rendererInfo_.streamUsage, GetRealUid(streamDesc));
        SetPlaybackStreamFlag(streamDesc);
    }
...
int32_t AudioCoreService::FetchRendererPipesAndExecute(std::vector<std::shared_ptr<AudioStreamDescriptor>> &streamDescs, const AudioStreamDeviceChangeReasonExt reason)
{
    AudioFlag audioFlag;
...
{
    // std::vector<std::shared_ptr<AudioStreamDescriptor>> inputStreamDescs = pipeManager_->GetAllInputStreamDescs(); // in plan : wait pipemananger
    std::vector<std::shared_ptr<AudioStreamDescriptor>> inputStreamDescs;
...
int32_t AudioCoreService::FetchRendererPipeAndExecute(std::shared_ptr<AudioStreamDescriptor> streamDesc,
    uint32_t &sessionId, AudioFlag &audioFlag, const AudioStreamDeviceChangeReasonExt reason)
{
...
void AudioCoreService::ProcessOutputPipeNew(std::shared_ptr<AudioPipeInfo> pipeInfo, AudioFlag &flag)
{
    uint32_t id = OpenNewAudioPortAndRoute(pipeInfo);
    ...
    // pipeManager_->AddPipeInfo(pipeInfo) in plan
}

void AudioCoreService::ProcessOutputPipeUpdate(std::shared_ptr<AudioPipeInfo> pipeInfo, AudioFlag &flag)
{
    for (auto desc : pipeInfo->streamDescriptors_) {
        switch (desc->streamAction_) {
            case AUDIO_STREAM_ACTION_NEW:
...
void AudioCoreService::ProcessInputPipeNew(std::shared_ptr<AudioPipeInfo> pipeInfo, AudioFlag &flag)
{
    uint32_t sourceId = OpenNewAudioPortAndRoute(pipeInfo);
...
void AudioCoreService::ProcessInputPipeUpdate(std::shared_ptr<AudioPipeInfo> pipeInfo, AudioFlag &flag)
{
    for (auto desc : pipeInfo->streamDescriptors_) {
        switch (desc->streamAction_) {
            case AUDIO_STREAM_ACTION_NEW:
...
{
    std::vector<std::shared_ptr<AudioPipeInfo>> pipeInfos;
    // std::vector<std::shared_ptr<AudioPipeInfo>> pipeInfos = pipeMananger_->GetUnusedPipe();
    for (auto pipeInfo : pipeInfos) {
        AUDIO_INFO_LOG("Close hid port id %{public}u", pipeInfo->id_);
        // AudioServerProxy::GetInstance().CloseAudioRoute(pipeInfo->id_); // In plan
        // pipeManager_->RemovePipeInfo(pipeInfo); in plan
    }
}

std::atomic<uint32_t> g_sessionId = {FIRST_SESSIONID}; // begin at 100000

uint32_t AudioCoreService::GenerateSessionId(int32_t uid)
{
...
void AudioCoreService::MoveToNewOutputDevice(std::shared_ptr<AudioStreamDescriptor> streamDesc, const AudioStreamDeviceChangeReasonExt reason)
{
    ...
    DeviceType oldDeviceType = streamDesc->oldDeviceDescs_.front()->deviceType_;
    ...
    if (audioConfigManager_.GetUpdateRouteSupport() && streamDesc->newDeviceDescs_.front()->networkId_ == LOCAL_NETWORK_ID &&
    ...
    // ResetOffloadAndMchMode(rendererChangeInfo, streamDesc->newDeviceDescs_);
...
void AudioCoreService::MoveToNewInputDevice(std::shared_ptr<AudioStreamDescriptor> streamDesc)
{
    ...
    if (audioConfigManager_.GetUpdateRouteSupport() && streamDesc->newDeviceDescs_.front()->networkId_ == LOCAL_NETWORK_ID) {
        audioActiveDevice_.UpdateActiveDeviceRoute(streamDesc->newDeviceDescs_.front()->deviceType_, DeviceFlag::INPUT_DEVICES_FLAG,
            streamDesc->newDeviceDescs_.front()->deviceName_);
...
void AudioCoreService::UpdateOutputRoute(std::shared_ptr<AudioStreamDescriptor> streamDesc)
{
    ...
    if (Util::IsRingerOrAlarmerStreamUsage(streamUsage) && IsRingerOrAlarmerDualDevicesRange(deviceType) &&
        ...
        if (ringerMode != RINGER_MODE_NORMAL && IsRingerOrAlarmerDualDevicesRange(streamDesc->newDeviceDescs_.front()->getType()) &&
...
{
    CHECK_AND_RETURN_RET_LOG(streamDesc->newDeviceDescs_.size() > 0 && streamDesc->newDeviceDescs_.size() <= AUDIO_CONCURRENT_ACTIVE_DEVICES_LIMIT, false,
    ...
    for (size_t i = 0; i < streamDesc->newDeviceDescs_.size(); i++) {
        if (IsRingerOrAlarmerDualDevicesRange(streamDesc->newDeviceDescs_[i]->deviceType_)) {
            activeDevices.push_back(make_pair(streamDesc->newDeviceDescs_[i]->deviceType_, DeviceFlag::OUTPUT_DEVICES_FLAG));
            AUDIO_INFO_LOG("select ringer/alarm devices devicetype[%{public}zu]:%{public}d", i, streamDesc->newDeviceDescs_[i]->deviceType_);
        } else {
...
void AudioCoreService::TriggerRecreateRendererStreamCallback(int32_t callerPid, int32_t sessionId,
    int32_t streamFlag, const AudioStreamDeviceChangeReasonExt::ExtEnum reason)
{
    ...
    int32_t streamFlag)
{
    ...
    AUDIO_INFO_LOG("Trigger recreate capturer stream, pid: %{public}d, sessionId: %{public}d, flag: %{public}d",
        callerPid, sessionId, streamFlag);
    if (audioPolicyServerHandler_ != nullptr) {
        audioPolicyServerHandler_->SendRecreateCapturerStreamEvent(callerPid, sessionId, streamFlag, AudioStreamDeviceChangeReasonExt::ExtEnum::UNKNOWN);
    } else {
        AUDIO_WARNING_LOG("No audio policy server handler");
    }
}
...
std::vector<std::shared_ptr<AudioDeviceDescriptor>> AudioCoreService::GetExcludedOutputDevices(
...
int32_t AudioCoreService::EventEntry::CreateRendererClient(std::shared_ptr<AudioStreamDescriptor> streamDesc, AudioFlag &flag, uint32_t &sessionId)
{
...
int32_t AudioCoreService::EventEntry::CreateCapturerClient(std::shared_ptr<AudioStreamDescriptor> streamDesc, AudioFlag &flag, uint32_t &sessionId)
{
    std::lock_guard<std::shared_mutex> lock(eventMutex_);
...
void AudioCoreService::EventEntry::OnServiceConnected(AudioServiceIndex serviceIndex)
{
    ...
    // load inner-cap-sink
    coreService_->LoadModernInnerCapSink();
#endif
...
vector<sptr<MicrophoneDescriptor>> AudioCoreService::EventEntry::GetAudioCapturerMicrophoneDescriptors(int32_t sessionId)
{
    std::shared_lock<std::shared_mutex> lock(eventMutex_);
...
void AudioCoreService::EventEntry::NotifyRemoteRenderState(std::string networkId, std::string condition, std::string value)
{
    std::lock_guard<std::shared_mutex> lock(eventMutex_);
...
std::vector<std::shared_ptr<AudioDeviceDescriptor>> AudioCoreService::EventEntry::GetExcludedOutputDevices(
    AudioDeviceUsage audioDevUsage)
{
    std::lock_guard<std::shared_mutex> lock(eventMutex_);
