...
int32_t AudioCoreService::FetchOutputDeviceAndRoute(const AudioStreamDeviceChangeReasonExt reason)
{
    ...
    for (auto streamDesc : outputStreamDescs) {
        ...
        AUDIO_INFO_LOG("DeviceType %{public}d, SessionID: %{public}u, state: %{public}u",
            streamDesc->newDeviceDescs_[0]->deviceType_, streamDesc->sessionId_, streamDesc->streamStatus_);
