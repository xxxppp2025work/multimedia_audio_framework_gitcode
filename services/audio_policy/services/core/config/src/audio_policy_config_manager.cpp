    ...
    const AudioStreamInfo &streamInfo, const StreamUsage &sreamUsage)
{
    ...
    CHECK_AND_RETURN_RET_LOG(stream.encoding != ENCODING_EAC3 || desc->deviceType_ == DEVICE_TYPE_HDMI ||
...
{
    for (auto info : infos) {
        if (info->format_ == streamInfo.format && info->sampleRate_ == streamInfo.samplingRate &&
