    ...
    std::unordered_map<std::string, std::shared_ptr<AdapterDeviceInfo>> tmpDeviceInfoMap_)
{
    if (deviceInfos.size() <= 0) {
        AUDIO_ERR_LOG("SetDeviceInfoMaps failed, deviceInfos is empty");
    }
...
void AudioPolicyConfigData::SetSupportDeviceAndPipeMaps(std::shared_ptr<AdapterPipeInfo> &pipeInfo_,
    std::unordered_map<std::string, std::shared_ptr<AdapterDeviceInfo>> tmpDeviceInfoMap_)
{
...
void AudioPolicyConfigData::Reorganize()
{
    ...
    for (auto &pair : adapterInfoMap) {
        SetDeviceInfoMaps(pair.second->deviceInfos, tmpDeviceInfoMap);
    }
    ...
    for (auto &pair : adapterInfoMap) {
        ...
        for (auto &pipeInfo_ : pair.second->pipeInfos) {
            SetSupportDeviceAndPipeMaps(pipeInfo_, tmpDeviceInfoMap);
        }
    }
...
std::shared_ptr<AdapterDeviceInfo> AudioPolicyConfigData::GetAdapterDeviceInfo(
    DeviceType type_, DeviceRole role_, const std::string &networkId, uint32_t flags)
{
    AUDIO_INFO_LOG("type_:%{public}d, role_:%{public}d, type_:%{public}s, type_:%{public}u", type_, role_,
        networkId.c_str(), flags);
    std::pair<DeviceType, DeviceRole> deviceMapKey = std::make_pair(type_, role);
    auto deviceInfoIt = deviceInfoMap.find(deviceMapKey);
    if (deviceInfoIt == deviceInfoMap.end()) {
    ...
    if (deviceInfoIt->second.size() == 1) {
        return deviceInfoIt->second.front();
    }
...
std::shared_ptr<AdapterDeviceInfo> PolicyAdapterInfo::GetDeviceInfoByType(eviceType deviceType, DeviceRole role)
{
