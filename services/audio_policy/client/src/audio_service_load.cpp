...
constexpr int ERR_READ_FAILED = -13;
...
int32_t AudioServiceLoad::LoadAudioService(void)
{
    ...
    sptr<AudioLoadCallback> AudioLoadCallback_(new AudioLoadCallback());
    int32_t ret = samgr->LoadSystemAbility(AUDIO_DISTRIBUTED_SERVICE_ID, AudioLoadCallback_);
    if (ret != SUCCESS) {
        isAudioServiceLoading_ = false;
        AUDIO_ERR_LOG("Load audio server SA failed, ret code:%{public}d", ret);
        return ret;
    }
    ret = samgr->LoadSystemAbility(AUDIO_POLICY_SERVICE_ID, AudioLoadCallback_);
    if (ret != SUCCESS) {
        isAudioServiceLoading_ = false;
