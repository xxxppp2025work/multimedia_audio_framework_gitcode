...
int32_t AudioPolicyProxy::SetSelfAppVolumeLevel(int32_t volumeLevel, int32_t volumeFlag = 0)
{
    MessageParcel data;
    MessageParcel reply;
    MessageOption option;
    ...
    int32_t error = Remote()->SendRequest(
    static_cast<uint32_t>(AudioPolicyInterfaceCode::SET_SELF_APP_VOLUMELEVEL), data, reply, option);
...
bool AudioPolicyProxy::IsAppVolumeMute(int32_t appUid, bool owned)
{
    ...
    int32_t error = Remote()->SendRequest(
        static_cast<uint32_t>(AudioPolicyInterfaceCode::GET_IS_APP_MUTE), data, reply, option);
...
{
    AUDIO_ERR_LOG("zhangjinqi Test --- 2");
    MessageParcel data;
    MessageParcel reply;
    MessageOption option;
    bool ret = data.WriteInterfaceToken(GetDescriptor());
...
int32_t AudioPolicyProxy::GetSelfAppVolumeLevel()
{
    ...
    CHECK_AND_RETURN_RET_LOG(ret, -1, "WriteInterfaceToken failed");
    int32_t error = Remote()->SendRequest(
        static_cast<uint32_t>(AudioPolicyInterfaceCode::GET_SELF_APP_VOLUMELEVEL), data, reply, option);
    CHECK_AND_RETURN_RET_LOG(error == ERR_NONE, error, "get volume failed, error: %d", error);
    return reply.ReadInt32();
}
