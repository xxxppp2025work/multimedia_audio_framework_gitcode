...
void AudioPolicyClientStubImpl::OnMicrophoneBlocked(const MicPhoneBlockedInfo &blockedInfo)
{
    std::lock_guard<std::mutex> lockCbMap(microphoneBlockedMutex_);
    MicPhoneBlockedInfo micPhoneBlockedInfo;
    micPhoneBlockedInfo.status = blockedInfo.status;
    for (auto it = microphoneBlockedCallbackList_.begin(); it != microphoneBlockedCallbackList_.end(); ++it) {
        micPhoneBlockedInfo.deviceDescriptors = blockedInfo.deviceDescriptors;
