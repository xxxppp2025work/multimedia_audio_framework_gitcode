...
class AudioWorkgroupCallbackProxy : public IRemoteProxy<IAudioWorkgroupCallback> {
public:
    explicit AudioWorkgroupCallbackProxy(const sptr<IRemoteObject>& impl);
    virtual ~AudioWorkgroupCallbackProxy();
    DISALLOW_COPY_AND_MOVE(AudioWorkgroupCallbackProxy);
    void OnWorkgroupChange(const AudioWorkgroupChangeInfo &info) override;

private:
    static inline BrokerDelegator<AudioWorkgroupCallbackProxy> delegator_;
