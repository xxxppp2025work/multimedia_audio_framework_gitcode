    ...
    EndpointType type)
{
    ...
    if (deviceInfo.deviceType_ == DEVICE_TYPE_BLUETOOTH_A2DP && deviceInfo.a2dpOffloadFlag_ != A2DP_OFFLOAD) {
        ...
        fastRenderId_ = manager.GetId(HDI_ID_BASE_RENDER, HDI_ID_TYPE_BLUETOOTH, HDI_ID_INFO_MMAP, true);
        return manager.GetRenderSink(fastRenderId_, true);
    }
    ...
    if (type == AudioEndpoint::TYPE_MMAP) {
        ...
        fastRenderId_ = manager.GetId(HDI_ID_BASE_RENDER, HDI_ID_TYPE_FAST, HDI_ID_INFO_VOIP, true);
        return manager.GetRenderSink(fastRenderId_, true);
    }
    return nullptr;
}
