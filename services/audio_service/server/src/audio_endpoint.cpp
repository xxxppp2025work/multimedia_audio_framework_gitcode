...
private:
    ...
    */
    ...
    bool CheckBuffer(uint8_t *buffer, size_t bufferSize, int32_t index);
...
bool AudioEndpointInner::CheckBuffer(uint8_t *buffer, size_t bufferSize, int32_t index)
{
...
void AudioEndpointInner::WriteMuteDataSysEvent(uint8_t *buffer, size_t bufferSize, int32_t index)
{
    ...
    if (CheckBuffer(buffer, bufferSize, index)) {
        ...
        if ((currentTime - tempProcess->GetStartMuteTime() >= ONE_MINUTE)
            && tempProcess->GetSilentState() == 1) { // 1 means unsilent
            tempProcess->SetSilentState(0); // 0 means silent
        ...
        if (tempProcess->GetSilentState() == 0) { // 0 means silent
            AUDIO_WARNING_LOG("begin write valid data in server");
            tempProcess->SetSilentState(1); // 1 means unsilent
