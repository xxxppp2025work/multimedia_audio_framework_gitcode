...
const uint64_t BUF_LENGTH_IN_MSEC_CAST = 10;
...
void PaRendererStreamImpl::PAStreamMovedCb(pa_stream *stream, void *userdata)
{
    ...
    if (!strcmp(deviceName, REMOTE_CAST_INNER_CAPTURER_SINK_NAME)) {
        rendererStreamImpl->remoteCastMovedFlag_ = true;
        rendererStreamImpl->UpdateBufferSize(BUF_LENGTH_IN_MSEC_CAST);
    } else if (rendererStreamImpl->remoteCastMovedFlag_) {
        rendererStreamImpl->remoteCastMovedFlag_ = false;
        rendererStreamImpl->UpdateBufferSize(BUF_LENGTH_IN_MSEC);
    }
