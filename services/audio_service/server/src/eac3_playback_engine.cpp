 ...
 */
#ifndef LOG_TAG
#define LOG_TAG "EAC3PlayBackEngine"
#endif
...
constexpr int32_t MAX_FAILURE_NUM = 50;
constexpr int32_t EAC3_STOP_TIMEOUT_IN_SEC = 8; // 8S
const std::string EAC3_SINK_NAME = "eac3";
const char *PRIMARY_ADAPTER_TYPE = "primary";

EAC3PlayBackEngine::EAC3PlayBackEngine()
    : isStart_(false),
      isInit_(false),
      failedCount_(0),
      latency_(0),
...
int32_t EAC3PlayBackEngine::Init(const AudioDeviceDescriptor &type, bool isVoip)
{
    if (!isInit_) {
        device_ = type;
        return SUCCESS;
...
int32_t EAC3PlayBackEngine::Start()
{
...
int32_t EAC3PlayBackEngine::Stop()
{
    ...
    AudioXCollie audioXCollie(
        "EAC3PlayBackEngine::Stop", EAC3_STOP_TIMEOUT_IN_SEC,
        [this](void *) { AUDIO_ERR_LOG("stop timeout"); }, nullptr,
...
int32_t EAC3PlayBackEngine::StopAudioSink()
{
    int32_t ret = SUCCESS;
    std::shared_ptr<IAudioRenderSink> sink = HdiAdapterManager::GetInstance().GetRenderSink(renderId_);
    if (sink && sink->IsInited()) {
...
int32_t EAC3PlayBackEngine::Pause()
{
    AUDIO_INFO_LOG("Enter");
    if (!isStart_) {
        AUDIO_INFO_LOG("already stopped");
    ...
    AudioXCollie audioXCollie(
        "EAC3PlayBackEngine::Pause", EAC3_STOP_TIMEOUT_IN_SEC,
        [this](void *) { AUDIO_ERR_LOG("stop timeout"); }, nullptr,
        AUDIO_XCOLLIE_FLAG_LOG | AUDIO_XCOLLIE_FLAG_RECOVERY);
    failedCount_ = 0;
...
int32_t EAC3PlayBackEngine::Flush()
{
    AUDIO_INFO_LOG("Enter");
    return SUCCESS;
}

void EAC3PlayBackEngine::DirectCallback(const RenderCallbackType type)
{
    switch (type) {
...
int32_t EAC3PlayBackEngine::RegisterWriteCallback()
{
    std::shared_ptr<IAudioRenderSink> sink = HdiAdapterManager::GetInstance().GetRenderSink(renderId_);
    CHECK_AND_RETURN_RET_LOG(sink != nullptr, ERR_INVALID_HANDLE, "null sink");
    CHECK_AND_RETURN_RET_LOG(sink->IsInited(), ERR_NOT_STARTED, "sink Not Inited! Init the sink first");
    ...
    std::function<void(const RenderCallbackType type)> callback =
        std::bind(&EAC3PlayBackEngine::DirectCallback, this, std::placeholders::_1);
    sink->RegistDirectHdiCallback(callback);
    return SUCCESS;
}

void EAC3PlayBackEngine::DoRenderFrame(std::vector<char> &audioBufferConverted, int32_t index, int32_t appUid)
{
    uint64_t written = 0;
    std::shared_ptr<IAudioRenderSink> sink = HdiAdapterManager::GetInstance().GetRenderSink(renderId_);
...
void EAC3PlayBackEngine::MixStreams()
{
    if (stream_ == nullptr) {
...
int32_t EAC3PlayBackEngine::AddRenderer(const std::shared_ptr<IRendererStream> &stream)
{
...
void EAC3PlayBackEngine::RemoveRenderer(const std::shared_ptr<IRendererStream> &stream)
{
...
bool EAC3PlayBackEngine::IsPlaybackEngineRunning() const noexcept
{
    return isStart_;
}

int32_t EAC3PlayBackEngine::GetDirectFormatByteSize(AudioSampleFormat format)
{
    switch (format) {
...
int32_t EAC3PlayBackEngine::InitSink(const AudioStreamInfo &clientStreamInfo)
{
...
int32_t EAC3PlayBackEngine::InitSink(uint32_t channel, AudioSampleFormat format, uint32_t rate,
...
uint64_t EAC3PlayBackEngine::GetLatency() noexcept
{
    return latency_;
}
} // namespace AudioStandard
