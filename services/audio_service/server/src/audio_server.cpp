...
int32_t AudioServer::DelCaptureNum(int32_t innerCapId)
{
    PlaybackCapturerManager *playbackCapturerMgr = PlaybackCapturerManager::GetInstance();
    if (playbackCapturerMgr->CheckReleaseUnloadModernInnerCapSink(innerCapId)) {
        PolicyHandler::GetInstance().UnloadModernInnerCapSink(innerCapId);
    }
    return SUCCESS;
}
#endif
