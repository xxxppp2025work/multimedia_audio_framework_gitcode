...
{
    uint32_t id = HdiAdapterManager::GetInstance().GetId(HDI_ID_BASE_CAPTURE, HDI_ID_TYPE_WAKEUP);
    std::shared_ptr<IAudioCaptureSource> wakeupSource = HdiAdapterManager::GetInstance().GetCaptureSource(id, true);
    if (wakeupSource != nullptr) {
        wakeupSource->RegistCallback(this);
    }

    uint32_t primaryId = HdiAdapterManager::GetInstance().GetId(HDI_ID_BASE_CAPTURE, HDI_ID_TYPE_PRIMARY);
    uint32_t usbId = HdiAdapterManager::GetInstance().GetId(HDI_ID_BASE_CAPTURE, HDI_ID_TYPE_PRIMARY,
        HDI_ADAPTER_ID_INFO_USB);
#ifdef SUPPORT_LOW_LATENCY
    uint32_t fastId = HdiAdapterManager::GetInstance().GetId(HDI_ID_BASE_CAPTURE, HDI_ID_TYPE_FAST);
    uint32_t voipFastId = HdiAdapterManager::GetInstance().GetId(HDI_ID_BASE_CAPTURE, HDI_ID_TYPE_FAST,
        HDI_ADAPTER_ID_INFO_VOIP);
#endif
    uint32_t btId = HdiAdapterManager::GetInstance().GetId(HDI_ID_BASE_CAPTURE, HDI_ID_TYPE_BLUETOOTH);

#ifdef SUPPORT_LOW_LATENCY
    for (auto id : { primaryId, usbId, fastId, voipFastId, btId }) {
#else
    for (auto id : { primaryId, usbId, btId }) {
#endif
        std::shared_ptr<IAudioCaptureSource> source = HdiAdapterManager::GetInstance().GetCaptureSource(id, true);
        if (source != nullptr) {
            source->RegistStateCallback(make_unique<CapturerStateOb>(
...
void AudioServer::RegisterAudioRendererSinkCallback()
{
    ...
    uint32_t primaryId = HdiAdapterManager::GetInstance().GetId(HDI_ID_BASE_RENDER, HDI_ID_TYPE_PRIMARY);
    uint32_t usbId = HdiAdapterManager::GetInstance().GetId(HDI_ID_BASE_RENDER, HDI_ID_TYPE_PRIMARY,
        HDI_ADAPTER_ID_INFO_USB);
    uint32_t directId = HdiAdapterManager::GetInstance().GetId(HDI_ID_BASE_RENDER, HDI_ID_TYPE_PRIMARY,
        HDI_ADAPTER_ID_INFO_DIRECT);
    uint32_t dpId = HdiAdapterManager::GetInstance().GetId(HDI_ID_BASE_RENDER, HDI_ID_TYPE_PRIMARY,
        HDI_ADAPTER_ID_INFO_DP);
    uint32_t voipId = HdiAdapterManager::GetInstance().GetId(HDI_ID_BASE_RENDER, HDI_ID_TYPE_PRIMARY,
        HDI_ADAPTER_ID_INFO_VOIP);
    uint32_t offloadId = HdiAdapterManager::GetInstance().GetId(HDI_ID_BASE_RENDER, HDI_ID_TYPE_OFFLOAD);
    uint32_t mchId = HdiAdapterManager::GetInstance().GetId(HDI_ID_BASE_RENDER, HDI_ID_TYPE_MULTICHANNEL);
    uint32_t btId = HdiAdapterManager::GetInstance().GetId(HDI_ID_BASE_RENDER, HDI_ID_TYPE_BLUETOOTH);
#ifdef SUPPORT_LOW_LATENCY
    uint32_t mmapBtId = HdiAdapterManager::GetInstance().GetId(HDI_ID_BASE_RENDER, HDI_ID_TYPE_BLUETOOTH,
        HDI_ADAPTER_ID_INFO_MMAP);
    uint32_t fastId = HdiAdapterManager::GetInstance().GetId(HDI_ID_BASE_RENDER, HDI_ID_TYPE_FAST);
    uint32_t voipFastId = HdiAdapterManager::GetInstance().GetId(HDI_ID_BASE_RENDER, HDI_ID_TYPE_FAST,
        HDI_ADAPTER_ID_INFO_VOIP);
#endif

#ifdef SUPPORT_LOW_LATENCY
    for (auto id : { primaryId, usbId, directId, dpId, voipId, offloadId, mchId, btId, mmapBtId, fastId, voipFastId }) {
#else
    for (auto id : { primaryId, usbId, directId, dpId, voipId, offloadId, mchId, btId }) {
#endif
