    ...
    uint32_t &spanSizeInframe, DeviceStreamInfo &serverStreamInfo, const std::shared_ptr<OHAudioBuffer> &buffer)
{
    ...
    serverStreamInfo.GetChannels(channelSet);
    ...
    CHECK_AND_RETURN_RET_LOG(serverStreamInfo.samplingRate.size() > 0 && channelSet.size() > 0,
    ...
    if (*channelSet.rbegin() != processConfig_.streamInfo.channels ||
        serverStreamInfo.format != processConfig_.streamInfo.format) {
        size_t spanSizeInByte = 0;
        if (processConfig_.audioMode == AUDIO_MODE_PLAYBACK) {
            uint32_t serverByteSize = *channelSet.rbegin() * PcmFormatToBits(serverStreamInfo.format);
