...
int32_t IpcStreamInServer::SetDefaultOutputDevice(int32_t defaultOutputDevice, bool notSet)
{
    if ((mode_ != AUDIO_MODE_PLAYBACK) || (rendererInServer_ == nullptr)) {
        AUDIO_ERR_LOG("mode is not playback or renderer is null");
        return ERR_OPERATION_FAILED;
    }
    return rendererInServer_->SetDefaultOutputDevice(static_cast<DeviceType>(defaultOutputDevice), notSet);
}
