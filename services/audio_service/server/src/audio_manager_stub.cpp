    ...
    MessageOption &option)
{
    switch (code) {
        ...
        case static_cast<uint32_t>(AudioServerInterfaceCode::DEL_CAPTURE_NUM):
            return HandleDelCaptureNum(data, reply);
#endif
        default:
...
int AudioManagerStub::HandleDelCaptureNum(MessageParcel &data, MessageParcel &reply)
{
    uint32_t innerCapId = data.ReadInt32();
    reply.WriteInt32(DelCaptureNum(innerCapId));
    return AUDIO_OK;
}
#endif
} // namespace AudioStandard
