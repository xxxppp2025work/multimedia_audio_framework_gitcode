    ...
    const std::unordered_map<int32_t, bool> &threads)
{
    for (const auto &tid : threads) {
        AUDIO_ERR_LOG("[WorkgroupInServer]set pid:%{public}d tid:%{public}d to qos_level7", pid, tid.first);
...
{
    for (const auto &tid : threads) {
        AUDIO_ERR_LOG("[WorkgroupInServer]set pid:%{public}d tid:%{public}d to qos%{public}d",
