...
public:
    ...
    uint32_t CreateSourcePort(HdiIdBase idBase, HdiIdType idType, const std::string &idInfo,
        const IAudioSourceAttr &attr) override;
    int32_t RegisterDataTransferCallback(const int32_t &pid, const sptr<IRemoteObject> &object) override;
    int32_t RegisterDataTransferMonitorParam(const int32_t &pid, const int32_t &callbackId,
        const DataTransferMonitorParam &param) override;
    int32_t UnregisterDataTransferMonitorParam(const int32_t &pid, const int32_t &callbackId) override;
