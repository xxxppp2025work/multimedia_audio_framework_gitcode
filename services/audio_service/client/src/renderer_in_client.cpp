...
bool RendererInClientInner::WriteCallbackFunc()
{
    ...
    futexRes = clientBuffer_->WaitFor(static_cast<int64_t>(timeout) * AUDIO_US_PER_SECOND,
        [this] () {
            ...
            uint32_t clientSizeInframe = static_cast<uint32_t>(cbBufferSize_ * sizePerFrameInByte_);
            int32_t writableInFrame = clientBuffer_ -> GetWritableDataFrames();
            if ((writableInFrame <= 0) || (clientSizeInframe > totalSizeInFrame) ||
                (writableInFrame < static_cast<int32_t>(totalSizeInFrame - clientSizeInframe))) {
                return false;
            }
    ...
    size_t oriBufferSize)
{
    ...
    RingBufferWrapper inBuffer = {
        .basicBufferDescs = {{
            {.buffer = buffer, .bufLength = bufferSize},
...
int32_t RendererInClientInner::WriteInner(uint8_t *buffer, size_t bufferSize)
{
    ...
    return WriteCacheData(buffer, bufferSize, speedCached, oriBufferSize);
}
