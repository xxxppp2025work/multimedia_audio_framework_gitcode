...
int32_t PolicyProviderWrapper::SetDefaultOutputDevice(int32_t defaultOutputDevice,
    const uint32_t sessionID, int32_t streamUsage, bool isRunning)
{
    CHECK_AND_RETURN_RET_LOG(policyWorker_ != nullptr, AUDIO_INIT_FAIL, "policyWorker_ is null");
    return policyWorker_->SetDefaultOutputDevice(static_cast<DeviceType>(defaultOutputDevice),
