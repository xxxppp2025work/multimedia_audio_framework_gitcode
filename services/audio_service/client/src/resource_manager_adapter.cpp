...
{
    AUDIO_INFO_LOG("ResourceManagerAdapter construct");
}

ResourceManagerAdapter::~ResourceManagerAdapter()
{
    AUDIO_INFO_LOG("ResourceManagerAdapter destroy");
}
...
{
    if (!resourceManager_) {
        AUDIO_INFO_LOG("InitResourceManager");
...
void ResourceManagerAdapter::RefreshResConfig()
{
    ...
    AUDIO_INFO_LOG("RefreshResConfig success");
}
...
void ResourceManagerAdapter::ReleaseSystemResourceManager()
{
    ...
    AUDIO_INFO_LOG("ReleaseSystemResourceManager success");
}
    ...
    std::unique_ptr<uint8_t[]> &outValue, uint32_t density)
{
    ...
    Global::Resource::RState = resourceManager_->GetMediaDataByName(name.c_str(), len, outValue);
    iAUDIO_INFO_LOG("rstate: %{public}d", static_cast<int32_t>(rstate));
    return rstate;
}
