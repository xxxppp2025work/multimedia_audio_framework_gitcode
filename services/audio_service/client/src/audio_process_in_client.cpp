...
public:
    ...
    int32_t SetDefaultOutputDevice(const DeviceType defaultOutputDevice, bool notSet = false) override;
...
int32_t AudioProcessInClientInner::SetDefaultOutputDevice(const DeviceType defaultOutputDevice, bool notSet)
{
    CHECK_AND_RETURN_RET_LOG(processProxy_ != nullptr, ERR_OPERATION_FAILED, "set failed with null ipcProxy.");
    return processProxy_->SetDefaultOutputDevice(defaultOutputDevice, notSet);
}
