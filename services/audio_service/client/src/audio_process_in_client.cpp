...
{
    AUDIO_INFO_LOG("WatchingRecordProcessCallbackFuc start");
    recordProcessCallbackFucThreadStatus_ = true;
    auto taskFunc = [this]() {
        if (recordProcessCallbackFucThreadStatus_) {
            AUDIO_INFO_LOG("WatchingRecordProcessCallbackFuc Set recordProcessCallbackFucThreadStatus_ to false");
            recordProcessCallbackFucThreadStatus_ = false;
        } else {
            AUDIO_INFO_LOG("watchdog happened, WatchingRecordProcessCallbackFuc process exit");
        }
    };
    ...
    AUDIO_INFO_LOG("WatchingRecordProcessCallbackFuc start %{public}d", sessionId_);
}

void AudioProcessInClientInner::WatchingProcessCallbackFuc()
{
    AUDIO_INFO_LOG("WatchingProcessCallbackFuc start");
    processCallbackFucThreadStatus_ = true;
    auto taskFunc = [this]() {
        if (processCallbackFucThreadStatus_) {
            AUDIO_INFO_LOG("WatchingProcessCallbackFuc Set processCallbackFucThreadStatus_ to false");
            processCallbackFucThreadStatus_ = false;
        } else {
            AUDIO_INFO_LOG("watchdog happened, WatchingProcessCallbackFuc process exit");
        }
    };
    ...
    HiviewDFX::Watchdog::GetInstance().RunPeriodicalTask(watchDogMessage, taskFunc,
        WATCHDOG_INTERVAL_TIME, WATCHDOG_DELAY_TIME);
    AUDIO_INFO_LOG("WatchingProcessCallbackFuc start %{public}d", sessionId_);
}
