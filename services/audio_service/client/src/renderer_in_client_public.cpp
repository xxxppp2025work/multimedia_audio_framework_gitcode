...
int32_t RendererInClientInner::SetDefaultOutputDevice(const DeviceType defaultOutputDevice, bool notSet)
{
    CHECK_AND_RETURN_RET_LOG(ipcStream_ != nullptr, ERR_ILLEGAL_STATE, "ipcStream is not inited!");
    int32_t ret = ipcStream_->SetDefaultOutputDevice(defaultOutputDevice, notSet);
    if (ret == SUCCESS) {
