...
int32_t AudioManagerProxy::DelCaptureNum(int32_t innerCapId)
{
    MessageParcel data;
    MessageParcel reply;
    MessageOption option;
    ...
    int32_t ret = Remote()->SendRequest(static_cast<uint32_t>(AudioServerInterfaceCode::DEL_CAPTURE_NUM),
        data, reply, option);
    CHECK_AND_RETURN_RET_LOG(ret == AUDIO_OK, ret, "Failed, ipc error: %{public}d", ret);
    ret = reply.ReadInt32();
    return ret;
