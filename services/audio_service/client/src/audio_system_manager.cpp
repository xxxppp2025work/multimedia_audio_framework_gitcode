...
int32_t AudioSystemManager::CreateAudioWorkgroup()
{
    ...
    if (res == SUCCESS) {
        workgroupPrioRecorderMap.emplace(workgroupId, std::make_shared<WorkgroupPrioRecorder>(workgroupId));
    }
    return workgroupId;
}
...
int32_t AudioSystemManager::ReleaseAudioWorkgroup(int32_t workgroupId)
{
    ...
    if (recorder != nullptr) {
        if (recorder->RestoreGroupPrio(false) != AUDIO_OK) {
            ...
            workgroupPrioRecorderMap.erase(workgroupId);
        }
    }
    ...
    const std::unordered_map<int32_t, bool> threads, bool &needUpdatePrio)
{
    ...
    if (needUpdatePrio || restoreByPermission) {
        const sptr<IStandardAudioService> gasp = GetAudioSystemManagerProxy();
        CHECK_AND_RETURN_RET_LOG(gasp != nullptr, ERR_INVALID_PARAM, "Audio service unavailable.");
        int32_t ipc_ret = gasp->ImproveAudioWorkgroupPrio(getpid(), threads);
        if (ipc_ret != SUCCESS) {
...
int32_t AudioSystemManager::WorkgroupPrioRecorder::RestoreThreadPrio(int32_t tokenId)
{
    ...
    if (it != threads_.end()) {
        ...
        ipc_ret = gasp->RestoreAudioWorkgroupPrio(getpid(), thread);
        if (ipc_ret != SUCCESS) {
...
{
    auto it = workgroupPrioRecorderMap.find(grpId);
    if (it != workgroupPrioRecorderMap.end()) {
        return it->second;
    }
    return nullptr;
}
