    ...
    const std::shared_ptr<AudioRendererDataTransferStateChangeCallback> &callback)
{
    ...
    if (g_audioListener == nullptr) {
        ...
        ret = gasp->RegisterDataTransferCallback(pid, object);
        CHECK_AND_RETURN_RET_LOG(ret == SUCCESS, ERROR_INVALID_PARAM, "ret: %{public}d", ret);
    }

    auto callbackId = g_audioListener->AddDataTransferStateChangeCallback(callback);
    ret = gasp->RegisterDataTransferMonitorParam(pid, callbackId, param);
    CHECK_AND_RETURN_RET_LOG(ret == SUCCESS, ERROR_INVALID_PARAM, "ret: %{public}d", ret);
    return ret;
}
    ...
    const std::shared_ptr<AudioRendererDataTransferStateChangeCallback> &callback)
{
    ...
    {
        gasp->UnregisterDataTransferMonitorParam(pid, callbackId);
    }
