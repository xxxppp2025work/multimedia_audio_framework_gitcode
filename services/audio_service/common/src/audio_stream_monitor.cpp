...
bool AudioStreamMonitor::HasRegistered(const int32_t pid, const int32_t callbackId)
{
    ...
    AUDIO_INFO_LOG("Monitoe not register, pid = %{public}d, callbackId = %{public}d", pid, callbackId);
    return false;
}
...
{
    std::lock_guard<std::recursive_mutex> lock(regStatusMutex_);
    ...
    const int32_t pid, const int32_t callbackId)
{
    ...
    for (auto iter = registerInfo_.begin(); iter != registerInfo_.end();) {
        if (iter->first.first == pid && iter->first.second == callbackId) {
            deleteUid = iter->second.clientUID;
            AUDIO_INFO_LOG("Unregister callback, deleteUid = %{public}d", deleteUid);
...
void AudioStreamMonitor::OnCallback(int32_t uid, std::shared_ptr<AudioRendererDataTransferStateChangeInfo> info)
{
    //To do callback
}
...
void AudioStreamMonitor::DeleteCheckForMonitor(uint32_t sessionId)
{
    ...
    for (auto iter = audioStreamCheckers_.begin(); iter != audioStreamCheckers_.end()) {
        if (iter->first == sessionId) {
            ...
            audioStreamCheckers_.erase(sessionId);
            return;
        }
    }
...
void AudioStreamMonitor::ReportStreamFreezen(int64_t interval)
{
    std::shared_ptr<Media::MediaMonitor::EventBean> bean = std::make_shared<Media::MediaMonitor::EventBean>(
        Media::MediaMonitor::AUDIO, Media::MediaMonitor::EventId::STREAM_FREEZEN,
        Media::MediaMonitor::EventType::FAULT_EVENT);
    bean->Add("TIME_INTERVAL", static_cast<uint64_t>(intervalTime));
