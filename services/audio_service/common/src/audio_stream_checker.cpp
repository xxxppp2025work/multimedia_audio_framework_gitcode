...
AudioStreamChecker::InitChecker(DataTransferMonitorParam para, const int32_t pid, const int32_t callbackId)
{
    std::lock_guard<std::recursive_mutex> lock(checkLock_);
    for (auto item : checkParaVector_) {
        if (item.callbackId == callbackId) {
            AUDIO_INFO_LOG("No need init check, callbackid = %{public}d", callbackId);
            return;
        }
    }
    ...
    if (isNeedCreateThread_.load()) {
        ...
        checkThread_ = std::thread([this] { this->CheckStreamThread(); });
        checkThread_.detach();
...
AudioStreamChecker::DeleteCheckerPara(const int32_t pid, const int32_t callbackId)
{
    std::lock_guard<std::recursive_mutex> lock(checkLock_);
...
void AudioStreamChecker::MonitorCheckFrameSub(CheckerParam &para)
{
    ...
    if (para.isMonitorMuteFrame) {
        ...
        AUDIO_DEBUG_LOG("Check mute frame size = %{public}ld", para.muteFrameNum);
    }
    if (para.isMonitorNoDataFrame) {
        abnormalFrameNum += para.noDataFrameNum;
        AUDIO_DEBUG_LOG("Check no data frame size = %{public}ld", para.noDataFrameNum);
    }
    if (timeCost < para.para.intervalTime) {
        AUDIO_DEBUG_LOG("Check time is not enough");
        return;
    }
    para.sumFrameCount = para.normalFrameCount + para.noDataFrameNum;
    float badFrameRatio = para.para.badFrameRatio / TRANS_PERCENTAGE;
    AUDIO_DEBUG_LOG("Check frame sum = %{public}ld, abnormal = %{public}ld, badRatio = %{public}f",
    ...
    if (abnormalFrameNum >= static_cast<int64_t>(para.sumFrameCount * badFrameRatio)) {
        ...
        if (para.lastStatus == DATA_TRANS_RESUME) {
            ...
            UDIO_DEBUG_LOG("sessionId = %{public}u, status change in DATA_TRANS_RESUME", streamConfig_.originalSessionId);
...
void AudioStreamChecker::MonitorOnAllCallback(DateTransferStateChangeType type)
{
    std::lock_guard<std::recursive_mutex> lock(checkLock_);
    if (!monitorSwitch_) {
...
void AudioStreamChecker::InitCallbackInfo(DateTransferStateChangeType type,
    AudioRendererDataTransferStateChangeInfo &callbackInfo)
{
...
void AudioStreamChecker::MonitorOnCallback(ateTransferStateChangeType type, bool isNeedCallback, CheckerParam &para)
{
    ...
    callbackInfo.badFrameRatio[NO_DATA_TRANS] = (para.noDataFrameNum * TRANS_INTEGER) / para.sumFrameCount;
    callbackInfo.badFrameRatio[SILENCE_DATA_TRANS] = (para.muteFrameNum * TRANS_INTEGER) / para.sumFrameCount;
    AUDIO_DEBUG_LOG("NO_DATA_TRANS ration = %{public}d, SILENCE_DATA_TRANS ratio = %{public}d",
        callbackInfo.badFrameRatio[NO_DATA_TRANS], callbackInfo.badFrameRatio[SILENCE_DATA_TRANS]);
    if (isNeedCallback) {
        // To do callback
    }
...
void AudioStreamChecker::RecordMuteFrame()
{
    ...
    for (int32_t index = 0; index < checkParaVector_.size(); index++) {
        ...
        AUDIO_DEBUG_LOG("Mute frame num = %{public}ld, callbackId = %{public}d",
...
void AudioStreamChecker::RecordNodataFrame()
{
    ...
    for (int32_t index = 0; index < checkParaVector_.size(); index++) {
        ...
        AUDIO_DEBUG_LOG("No data frame num = %{public}ld, callbackId = %{public}d",
...
void AudioStreamChecker::RecordNormalFrame()
{
    ...
    for (int32_t index = 0; index < checkParaVector_.size(); index++) {
        ...
        AUDIO_DEBUG_LOG("Normal frame num = %{public}ld, callbackId = %{public}d",
