...
{
    CHECK_AND_RETURN_LOG(statusInfo_ != nullptr, "statusInfo is nullptr");
    auto futex = GetFutex();
    while (true) {
        uint32_t expected = LOCK_RELEASED;
        if (futex->compare_exchange_weak(expected, LOCK_OWNED)) {
            break;
        }
...
{
    CHECK_AND_RETURN_RET_LOG(statusInfo_ != nullptr, "statusInfo is nullptr");
    auto futex = GetFutex();
    while (true) {
        uint32_t expected = LOCK_RELEASED;
        if (futex->compare_exchange_weak(expected, LOCK_OWNED)) {
...
size_t VASharedBufferOperator::Read(uint8_t *data, size_t dataSize)
{
    ...
    // if writeSize doesn't reach the margin of buffer
