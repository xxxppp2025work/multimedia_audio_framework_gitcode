...
int32_t OHAudioBufferBase::GetBufferByOffset(size_t offset, size_t dataLenth, RingBufferWrapper &buffer)
{
    ...
    CHECK_AND_RETURN_RET_LOG(dataLenth <= totalSizeInByte_, ERR_INVALID_PARAM,
...
int32_t OHAudioBufferBase::GetAllWritableBuffer(uint64_t writePosInFrame, RingBufferWrapper &buffer)
{
    uint64_t basePos = basicBufferInfo_->basePosInFrame.load();
    uint64_t readPos = basicBufferInfo_->curReadFrame.load();
    uint64_t maxWriteDelta = 2 * totalSizeInFrame_; // 0 ~ 2*totalSizeInFrame_
...
int32_t OHAudioBufferBase::GetAllReadableBuffer(uint64_t readPosInFrame, RingBufferWrapper &buffer)
{
    uint64_t basePos = basicBufferInfo_->basePosInFrame.load();
