...
{
    // ignore stdout, stdin and stderr.
...
int32_t VASharedBuffer::Init(const VASharedMemInfo& memInfo)
{
    ...
    if (memInfo.statusFd_ > MINFD && memInfo.statusMemCapacity_ > 0) {
        ...
        statusInfoMem_ = VAAudioSharedMemory::CreateFromRemote(memInfo.statusFd_, memInfo.statusMemCapacity_, STATUS_INFO_BUFFER);
    } else {
...
{
    if (memInfo.dataFd_ <= MINFD || memInfo.statusFd_ <= MINFD || memInfo.dataMemCapacity_ <= 0 || memInfo.statusMemCapacity_ <= 0) {
        return nullptr;
    }
