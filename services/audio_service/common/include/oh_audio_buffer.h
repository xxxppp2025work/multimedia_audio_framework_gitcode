...
public:
    ...
    int32_t GetAllWritableBuffer(uint64_t writePosInFrame, RingBufferWrapper &buffer);
    int32_t GetAllReadableBuffer(uint64_t readPosInFrame, RingBufferWrapper &buffer);
...
private:
    ...
    const uint32_t byteSizePerFrame_;
...
private:
    ...
    const SpanBasicInfo spanBasicInfo_;
