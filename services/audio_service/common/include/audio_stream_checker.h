...
struct CheckerParam {
    ...
    DateTransferStateChangeType lastStatus = DATA_TRANS_RESUME;
};
...
public:
    ...
    void MonitorOnCallback(DateTransferStateChangeType type, bool isNeedCallback, CheckerParam &para);
    void MonitorOnAllCallback(DateTransferStateChangeType type);
    int32_t GetAppUid();
...
private:
    ...
    bool IsMonitorNoDataFrame(const ChecerkParam &para);
    void InitCallbackInfo(DateTransferStateChangeType type, AudioRendererDataTransferStateChangeInfo &callbackInfo);
